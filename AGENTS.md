...
3. **WC markets get seeded by `wc-demo-seed.ts` at boot** so the home
   ...
   **The "Top forecasters" leaderboard gets seeded too** by
   `leaderboard-demo-seed.ts` (R-UAT-FN-19.1) right after the WC seed.
   Same idempotency model: 5 demo addresses only, current UTC week
   only, `INSERT OR REPLACE` so re-runs converge. Real users (any
   other address) are never touched. To remove the demo rows
   without a service restart: `pnpm --filter @suipredict/agents
   seed:leaderboard:clear`.
...
| Web UI shows "No DeepBook pool for this market" for a market that the maker should be able to quote | The SQLite mirror row has `deepbook_pool_id IS NULL` (or stale from a previous package) | R-WC-1.8: the new `PoolProvisioner` agent (`*/15 * * * *`) auto-mints the missing pool + `PredictionMarket` via `ensureMarketCreated` and writes the ids back to SQLite. The amber warning disappears on the next page load. For ghost rows the circuit-breaker already tripped on, the provisioner correctly skips (no point retrying until the contract is upgraded to per-market coin types). |
| Agent wallet runs out of DUSDC mid-session | No auto-funder; the maker's per-tick `authorize_spend` keeps consuming budget but orders don't land | R-WC-1.8: the new `AutoFunder` agent (`*/10 * * * *`) auto-mints DUSDC via the self-hosted TreasuryCap when the balance drops below the configured floor. For DEEP (no in-agent mint path on this deployment), the funder surfaces a clear `NEEDS FUNDING` noop pointing at the testnet DEEP swap. |
...
| **R-UAT-FN-19.1** | **Fix** | **Top Forecasters mock data seeds automatically at agent-service boot.** The pre-fix state showed "No forecasters on the board yet" on every fresh deploy / API-only mode because `daily_scores` was empty until real users participated. The new `apps/agents/src/agents/leaderboard-demo-seed.ts` runs on boot right after `seedWcDemoMarkets()`, writing 5 demo forecasters (deployer + 4 well-formed placeholder addresses) with realistic Move-semantics streak patterns: 7.07 / 6.06 / 6.03 / 5.03 / 4.03. Includes (a) two new public helpers `recordDailyScores(scores[])` + `clearDailyScoresForUsersInRange(users, start, end)` on `gamification/store.ts` for atomic batch writes/deletes, (b) automatic legacy-placeholder cleanup for the pre-TS `scripts/seed-leaderboard.mjs` addresses (`0xa1b2…` through `0xd4e5…`) — opt out with `SUPPRESS_LEGACY_DEMO_CLEANUP=1`, and (c) operator script `pnpm --filter @suipredict/agents seed:leaderboard:clear` for manual rollback. Real users (any address outside the 5 known demo addresses) are NEVER touched. 8/8 unit tests pass (`tests/leaderboard-demo-seed.test.ts`).** | **`apps/agents/src/agents/leaderboard-demo-seed.ts`, `apps/agents/src/gamification/store.ts`, `apps/agents/src/index.ts`, `apps/agents/scripts/seed-leaderboard.mjs`, `apps/agents/package.json`, `apps/agents/tests/leaderboard-demo-seed.test.ts`, `README.md`, `AGENTS.md`** |
...
| **R-WC-1.8** | **Feature** | **Auto DeepBook-pool provisioner + DUSDC auto-funder.** Two new agents close the deployment gap surfaced by the production audit (40 of 43 markets showed the "No DeepBook pool for this market" amber warning). `AutoFunder` (`*/10 * * * *`) keeps the agent wallet topped up with DUSDC via the self-hosted `TreasuryCap`'s `coin::mint_and_transfer` PTB (same pattern as `scripts/topup-bm-dusdc.ts`); DEEP has no in-agent mint path on this deployment (Sui system coin), so a DEEP shortfall surfaces a clear `NEEDS FUNDING` `noop` pointing at the testnet DEEP swap with the canonical `0x7b86477f…::deep::DEEP` type. `PoolProvisioner` (`*/15 * * * *`) scans the SQLite mirror for active markets with no `deepbook_pool_id`, excludes `wc26-*` ghost rows that already tripped the R-WC-1.2 circuit-breaker, pre-flight balances via the same pattern as `world-cup-creator.ts`, and calls the SDK's `ensureMarketCreated` (already handles R-WC-1.7's EPoolAlreadyExists recovery). Persists the new pool + on-chain ids via `upsertMarket`. New helper `countMarketsWithOnchainByPackage` in `markets/store.ts` dereferences each row's `onchain_market_id` via `sui_getObject` to detect "first market on this package" — only the first market pays 500 DEEP (`POOL_CREATION_FEE_DEEP`); subsequent markets reuse the pool via `create_market_with_pool`. Three new env vars (`AUTO_FUNDER_MIN_DUSDC_ATOMS`, `AUTO_FUNDER_TOPUP_DUSDC_ATOMS`, `AUTO_FUNDER_MIN_DEEP_ATOMS`) all optional with sensible defaults (100 USDC floor, 10,000 USDC topup, 500 DEEP floor).** | **`apps/agents/src/agents/auto-funder.ts`, `apps/agents/src/agents/pool-provisioner.ts`, `apps/agents/src/markets/store.ts`, `apps/agents/src/index.ts`, `README.md`, `AGENTS.md`** |
