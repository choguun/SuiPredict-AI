...
// v1.5 additive upgrade. Extractors for StreakRegistry (shared) and
// UserStreak (per-user owned). Consumed by v2's migrate.move.

/// Extract the StreakRegistry's full table as (address, ID) pairs.
/// Note: Tables in Move don't expose iteration; we rely on the
/// admin having already called `streak_id_for` to collect keys
/// before migration. If the registry is empty (no users), the
/// v2 reconstruction creates an empty Table.
public fun extract_streak_registry(registry: StreakRegistry): Table<address, ID> {
    let StreakRegistry { id, streaks } = registry;
    object::delete(id);
    streaks
}

/// Extract a single UserStreak by value. Returns all fields so v2's
/// migrate.move can reconstruct with the same state.
public fun extract_user_streak(streak: UserStreak): (
    address, // owner
    u64, u64, u64, // current, longest, last_day
    bool, u64, u64, // has_participated, total_participated, total_correct
    u8, vector<bool>, u8, // multiplier_tier, claimed_tiers, market_category
) {
    let UserStreak {
        id,
        owner,
