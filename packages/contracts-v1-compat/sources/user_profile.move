...
// v1.5 additive upgrade. Extracts a single UserProfile by value.
public fun extract_user_profile(profile: UserProfile): (
    address, // owner
    vector<u8>, // country_code
    u8, // forecaster_kind
    u64, // created_at_ms
) {
    let UserProfile {
    ...
    object::delete(id);
    (owner, country_code, forecaster_kind, created_at_ms)
}

/// Extract the ProfileRegistry's table for migration.
public fun extract_profile_registry(registry: ProfileRegistry): Table<address, ID> {
