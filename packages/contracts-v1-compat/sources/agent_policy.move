...
// Added in v1.5 additive upgrade. Read-only accessors that return
// the live shared object's field values as a tuple so the v2
// fresh-publish package's `migrate.move` can consume the v1 object
// by value and reconstruct a v2-typed AgentPolicy with the same
// fields. The v1 object is consumed (move-by-value into the tuple
// extractor) and its fields are returned as primitives — no v1
// state persists beyond this call.
public fun extract_agent_policy(policy: AgentPolicy): (
    address, // owner
    address, // agent
    u64,     // max_budget
    u64,     // spent
    ...
    bool,    // revoked
    bool,    // paused
    ID,      // v1 object id (for logging / cross-check)
) {
    let AgentPolicy {
        id,
        owner,
    ...
    object::delete(id);
    (owner, agent, max_budget, spent, expires_at, revoked, paused, object::id_from_address(@0x0))
}

public fun extract_agent_policy_keep(policy: &AgentPolicy): (
    address, address, u64, u64, u64, bool, bool,
) {
    (
        policy.owner, policy.agent, policy.max_budget, policy.spent,
        policy.expires_at, policy.revoked, policy.paused,
    )
}
