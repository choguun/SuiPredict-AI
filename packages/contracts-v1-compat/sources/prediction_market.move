...
// === R-WC-3.1 migration extractor ===
// v1.5 additive upgrade. Extracts a FeeVault<Q> by value. Returns
// the admin address and the balance value (in MIST). The v1 vault
// is consumed; the caller reconstructs a v2 FeeVault with the same
// admin and re-deposits the balance.
public fun extract_fee_vault<Q>(vault: FeeVault<Q>): (address, u64) {
    let FeeVault { id, admin, fee_balance } = vault;
    object::delete(id);
...
// === R-WC-3.1 deposit helper for v2 migration ===
/// Creates a FeeVault<Q> from an incoming Coin<Q>. Used by the
/// v2 migration PTB after `extract_fee_vault` returns the balance
/// value — the migration PTB mints (or transfers) a Coin<Q> of
/// that amount and passes it here.
public fun init_fee_vault_with_coin<Q>(
    admin: address,
    coin: Coin<Q>,
    ctx: &mut TxContext,
) {
