...
import { AGENT_POLICY_PACKAGE_ID, CLOCK_OBJECT_ID, DUSDC_TYPE, resolveAgentPolicyPackageId, } from "./constants.js";
import { encodeUtf8 } from "./markets/constants.js";
import { DEEPBOOK_REGISTRY_ID, } from "./deepbook/constants.js";
import { buildDeepBookPlaceLimitOrderTx, buildDeepBookWithdrawSettledTx, createPredictionDeepBookClient, getOrderBookDepth, getMidPrice, PREDICT_DEEPBOOK_POOL_KEY, } from "./deepbook/client.js";
 ...
 */
export const PREDICT_MARKET_PACKAGE_ID = (process.env.NEXT_PUBLIC_MARKET_PACKAGE_ID ??
...
function isPowerOf10(n) {
    if (n <= 0n)
        return false;
    const POWERS = [
        1n, 10n, 100n, 1000n, 10000n, 100000n, 1000000n,
        10000000n, 100000000n, 1000000000n, 10000000000n,
        100000000000n, 1000000000000n, 10000000000000n,
        100000000000000n, 1000000000000000n,
        10000000000000000n, 1000000000000000000n,
    ];
 ...
 */
export const FEE_VAULT_ID = (process.env.NEXT_PUBLIC_FEE_VAULT_ID ??
 ...
 */
export const SHARED_TREASURY_HOLDER_ID = (process.env.NEXT_PUBLIC_SHARED_TREASURY_HOLDER_ID ??
 ...
 */
export const REFERRAL_TREASURY_ADDRESS = (process.env.NEXT_PUBLIC_REFERRAL_TREASURY_ADDRESS ??
...
export function buildCreateMarketTx(params) {
    ...
    if (typeof params.expiryMs !== "bigint") {
        throw new Error(`buildCreateMarketTx: expiryMs must be bigint, got ${typeof params.expiryMs}`);
    }
    if (params.expiryMs <= 0n) {
        throw new Error(`buildCreateMarketTx: expiryMs must be > 0 (got ${params.expiryMs})`);
    }
    if (params.expiryMs <= BigInt(Date.now())) {
        throw new Error(`buildCreateMarketTx: expiryMs must be in the future ` +
    ...
    if (titleBytes > MAX_TITLE_BYTES) {
        throw new Error(`buildCreateMarketTx: title length ${titleBytes} bytes exceeds ${MAX_TITLE_BYTES} (max)`);
    }
    ...
    if (resBytes > MAX_RESOLUTION_SOURCE_BYTES) {
        throw new Error(`buildCreateMarketTx: resolutionSource length ${resBytes} bytes exceeds ${MAX_RESOLUTION_SOURCE_BYTES} (max)`);
    }
    ...
    if (!Number.isInteger(category) || category < 0 || category > 3) {
        throw new Error(`buildCreateMarketTx: category must be 0..3 (0=none, 1=AI, 2=crypto, 3=other), got ${category}`);
    }
    ...
    // can match against.
    const tickSize = params.tickSize ?? 1000000n;
    const lotSize = params.lotSize ?? 1000000n;
    const minSize = params.minSize ?? 1000000n;
    if (tickSize <= 0n) {
    ...
    if (!isPowerOf10(tickSize)) {
        throw new Error(`buildCreateMarketTx: tickSize must be a power of 10 (got ${tickSize})`);
    }
    if (lotSize < 1000n || !isPowerOf10(lotSize)) {
        throw new Error(`buildCreateMarketTx: lotSize must be a power of 10 >= 1000 (got ${lotSize})`);
    }
    if (!isPowerOf10(minSize)) {
        throw new Error(`buildCreateMarketTx: minSize must be a power of 10 (got ${minSize})`);
    }
    if (minSize % lotSize !== 0n) {
        throw new Error(`buildCreateMarketTx: minSize (${minSize}) must be a multiple of lotSize (${lotSize})`);
    }
...
export function buildCreateMarketWithPoolTx(params) {
    ...
    if (typeof params.expiryMs !== "bigint") {
        throw new Error(`buildCreateMarketWithPoolTx: expiryMs must be bigint, got ${typeof params.expiryMs}`);
    }
    if (params.expiryMs <= 0n) {
        throw new Error(`buildCreateMarketWithPoolTx: expiryMs must be > 0 (got ${params.expiryMs})`);
    }
    if (params.expiryMs <= BigInt(Date.now())) {
        throw new Error(`buildCreateMarketWithPoolTx: expiryMs must be in the future ` +
            `(got ${params.expiryMs}, now=${BigInt(Date.now())})`);
    }
    const MAX_TITLE_BYTES = 256;
    const MAX_RESOLUTION_SOURCE_BYTES = 1024;
    ...
    const titleBytes = encodeUtf8(params.title).length;
    if (titleBytes > MAX_TITLE_BYTES) {
        throw new Error(`buildCreateMarketWithPoolTx: title length ${titleBytes} bytes exceeds ${MAX_TITLE_BYTES} (max)`);
    }
    if (!params.resolutionSource || !params.resolutionSource.trim()) {
    ...
    const resBytes = encodeUtf8(params.resolutionSource).length;
    if (resBytes > MAX_RESOLUTION_SOURCE_BYTES) {
        throw new Error(`buildCreateMarketWithPoolTx: resolutionSource length ${resBytes} bytes exceeds ${MAX_RESOLUTION_SOURCE_BYTES} (max)`);
    }
    // R52 audit fix: clamp `category` to [0, 3] like
    // buildCreateMarketTx.
    const category = params.category ?? 0;
    if (!Number.isInteger(category) || category < 0 || category > 3) {
        throw new Error(`buildCreateMarketWithPoolTx: category must be 0..3, got ${category}`);
    }
 ...
 */
export function buildMintSharesTx(marketId, vaultId, quoteIn, amountAtoms, 
/**
...
sharedCapsId) {
    ...
    if (!/^0x[0-9a-fA-F]{64}$/.test(marketId)) {
        throw new Error(`buildMintSharesTx: marketId "${marketId}" is not a valid Sui object id (expected 0x + 64 hex chars). ` +
    ...
    if (!/^0x[0-9a-fA-F]{64}$/.test(vaultId)) {
        throw new Error(`buildMintSharesTx: vaultId "${vaultId}" is not a valid Sui object id (expected 0x + 64 hex chars). ` +
    ...
    if (!/^0x[0-9a-fA-F]{64}$/.test(quoteIn)) {
        throw new Error(`buildMintSharesTx: quoteIn "${quoteIn}" is not a valid Sui object id (expected 0x + 64 hex chars). ` +
    ...
    const MIN_ATOMS = 1000000n;
    if (amountAtoms <= 0n) {
        throw new Error(`buildMintSharesTx: amountAtoms must be > 0 (got ${amountAtoms})`);
    }
    if (amountAtoms < MIN_ATOMS) {
        throw new Error(`buildMintSharesTx: amountAtoms ${amountAtoms} is below the protocol minimum of ${MIN_ATOMS} (1 YES share). ` +
...
export function buildMintSharesBatchTx(params) {
    ...
    if (params.amountPerMarket <= 0n) {
        throw new Error(`buildMintSharesBatchTx: amountPerMarket must be > 0 (got ${params.amountPerMarket})`);
    }
    if (params.amountPerMarket < MIN_ATOMS) {
        throw new Error(`buildMintSharesBatchTx: amountPerMarket ${params.amountPerMarket} ` +
    ...
    const [primaryCoin, ...splitCoins] = tx.splitCoins(tx.object(normalizeObjectId(params.quoteIn)), amounts);
    const coins = [primaryCoin, ...splitCoins];
    if (coins.length !== params.marketIds.length) {
        throw new Error(`buildMintSharesBatchTx: splitCoins produced ${coins.length} coins for ${params.marketIds.length} markets`);
    }
 ...
 */
export function buildResolveMarketTx(marketId, outcome, 
/**
 * R-WC-2: per-market phantom `M: address`. Threaded as the
...
m) {
    ...
    if (outcome !== 1 && outcome !== 2) {
        throw new Error(`buildResolveMarketTx: outcome must be 1 (YES) or 2 (NO), got ${outcome}`);
    }
    const tx = new Transaction();
    tx.moveCall({
 ...
 */
export function buildRedeemTx(marketId, vaultId, winningCoin, 
/**
 * R-WC-2: per-market phantom `M: address`. Threaded as the
...
export function buildDisputeMarketTx(marketId, evidenceUri, 
/**
 * R-WC-2: per-market phantom `M: address`. Threaded as the
 * second type argument to `dispute_market<Q, M>`.
 */
m) {
    const evidence = typeof evidenceUri === "string" ? encodeUtf8(evidenceUri) : evidenceUri;
    if (evidence.length === 0) {
        throw new Error("buildDisputeMarketTx: evidenceUri must be non-empty (on-chain " +
            "check is vector::length > 0, see EZeroAmount in " +
            "prediction_market.move)");
    }
    if (evidence.length > MAX_EVIDENCE_URI_BYTES) {
        throw new Error(`buildDisputeMarketTx: evidenceUri is ${evidence.length} bytes; ` +
 ...
 */
export function buildResolveDisputeTx(marketId, finalOutcome, 
/**
 * R-WC-2: per-market phantom `M: address`. Threaded as the
...
m) {
    ...
    if (finalOutcome !== 1 && finalOutcome !== 2) {
        throw new Error(`buildResolveDisputeTx: finalOutcome must be 1 (YES) or 2 (NO), got ${finalOutcome}`);
    }
    const tx = new Transaction();
    tx.moveCall({
 ...
 */
export function buildRedeemNoTx(marketId, vaultId, winningCoin, 
/**
 * R-WC-2: per-market phantom `M: address`. Threaded as the
 ...
 */
export function buildRedeemWithStreakTx(marketId, vaultId, winningCoinId, streakId, 
/**
 * R-WC-2: per-market phantom `M: address`. Threaded as the
...
export function buildRedeemNoWithStreakTx(marketId, vaultId, winningCoinId, streakId) {
    const tx = new Transaction();
    tx.moveCall({
 ...
 */
export function buildSetupReferralTx(marketId, poolId, multiplier = 1000000000n, 
/**
 * R-WC-2: per-market phantom `M: address`. Threaded as the
...
m) {
    ...
    if (typeof multiplier !== "bigint" || multiplier <= 0n) {
        throw new Error(`buildSetupReferralTx: multiplier must be a bigint > 0 (got ${multiplier})`);
    }
    const tx = new Transaction();
    tx.moveCall({
 ...
 */
export function buildClaimReferralRewardsTx(poolId, referralId, 
/**
 * R-WC-2: per-market phantom `M: address`. Threaded as the
...
m) {
    ...
    if (normalizeObjectId(poolId) === normalizeObjectId(referralId)) {
        throw new Error("buildClaimReferralRewardsTx: poolId and referralId must differ; " +
 ...
 */
export function buildWithdrawFeesTx(vaultId, amount) {
    ...
    if (amount <= 0n) {
        throw new Error(`buildWithdrawFeesTx: amount must be > 0 (got ${amount})`);
    }
    const tx = new Transaction();
    tx.moveCall({
 ...
 */
export function buildInitFeeVaultTx(adminCapId, vaultAdmin) {
    ...
    if (!isValidSuiAddress(vaultAdmin)) {
        throw new Error(`buildInitFeeVaultTx: vaultAdmin must be a non-zero Sui address (got "${vaultAdmin}")`);
    }
    const tx = new Transaction();
    tx.moveCall({
...
// vault_admin: signer`.
export function buildInitFeeVaultFallbackTx() {
    const tx = new Transaction();
    tx.moveCall({
 ...
 */
export async function getMarketOrderBook(dbClient, poolId, lowPrice = 0.01, highPrice = 0.99) {
    return getOrderBookDepth(dbClient, poolId, lowPrice, highPrice);
}
/**
 * Get mid price from DeepBook pool.
 */
export async function getMarketMidPrice(dbClient, poolId) {
 ...
 */
export function marketTypeSeed(seed) {
 ...
 */
export function withMarketType(tx, m) {
    ...
    console.warn("[withMarketType] DEPRECATED: this post-processor can't mutate " +
 ...
 */
export function yesCoinType(packageId = PKG(), m) {
    const suffix = m ? `, ${addressOf(m)}` : "";
    return `${packageId}::prediction_market::YES<${DUSDC_TYPE}${suffix}>`;
}
export function noCoinType(packageId = PKG(), m) {
    const suffix = m ? `, ${addressOf(m)}` : "";
 ...
 */
export function buildPlaceYesLimitOrderTx(dbClient, poolKey, params) {
    ...
    if (!(params.price > 0)) {
        throw new Error(`buildPlaceYesLimitOrderTx: price must be > 0, got ${params.price}`);
    }
    if (!(params.quantity > 0)) {
        throw new Error(`buildPlaceYesLimitOrderTx: quantity must be > 0, got ${params.quantity}`);
    }
 ...
 */
export function buildWithdrawSettledTx(dbClient, poolKey) {
 ...
 */
export function buildMarketWithdrawSettledTx(marketId, poolId, balanceManagerId) {
    const tx = new Transaction();
    tx.moveCall({
...
export function buildPlaceMarketOrderTx(params) {
    ...
    if (params.quantity <= 0n) {
        throw new Error(`buildPlaceMarketOrderTx: quantity must be > 0, got ${params.quantity}`);
    }
    if (params.clientOrderId < 0n) {
        throw new Error(`buildPlaceMarketOrderTx: clientOrderId must be >= 0, got ${params.clientOrderId}`);
    }
    const tx = new Transaction();
    tx.moveCall({
...
export function buildPlaceOrderTx(params) {
    ...
    if (!/^0x[0-9a-fA-F]{64}$/.test(params.marketId)) {
        throw new Error(`buildPlaceOrderTx: marketId "${params.marketId}" is not a valid Sui object id (expected 0x + 64 hex chars). ` +
    ...
    if (!/^0x[0-9a-fA-F]{64}$/.test(params.poolId)) {
        throw new Error(`buildPlaceOrderTx: poolId "${params.poolId}" is not a valid Sui object id (expected 0x + 64 hex chars). ` +
    ...
    if (!/^0x[0-9a-fA-F]{64}$/.test(params.balanceManagerId)) {
        throw new Error(`buildPlaceOrderTx: balanceManagerId "${params.balanceManagerId}" is not a valid Sui object id (expected 0x + 64 hex chars). ` +
    ...
    // confusion).
    if (params.price <= 0n) {
        throw new Error(`buildPlaceOrderTx: price must be > 0, got ${params.price}`);
    }
    if (params.quantity <= 0n) {
        throw new Error(`buildPlaceOrderTx: quantity must be > 0, got ${params.quantity}`);
    }
    if (params.clientOrderId < 0n) {
        throw new Error(`buildPlaceOrderTx: clientOrderId must be >= 0, got ${params.clientOrderId}`);
    }
    ...
    if (params.clientOrderId > 0xffffffffffffffffn) {
        throw new Error(`buildPlaceOrderTx: clientOrderId must fit in u64 (got ${params.clientOrderId})`);
    }
    ...
    if (params.orderType !== undefined &&
        ...
        throw new Error(`buildPlaceOrderTx: orderType must be in [0, 3], got ${params.orderType}`);
    }
    const tx = new Transaction();
    tx.moveCall({
 ...
 */
export const QUOTE_SCALE = 1000000000n;
/**
 ...
 */
export const BASE_SCALE = 1000000n;
// ─── Cancel wrappers ────────────────────────────────────────────────────────
/**
...
export function buildCancelOrderTx(params) {
    ...
    if (params.orderId <= 0n) {
        throw new Error(`buildCancelOrderTx: orderId must be > 0 (got ${params.orderId})`);
    }
    if (params.orderId > 0xffffffffffffffffffffffffffffffffn) {
        throw new Error(`buildCancelOrderTx: orderId must fit in u128 (got ${params.orderId})`);
    }
    const tx = new Transaction();
    tx.moveCall({
...
export function buildCancelOrdersTx(params) {
    ...
    if (params.orderIds.length === 0) {
        throw new Error("buildCancelOrdersTx: orderIds must be non-empty; use buildCancelAllOrdersTx for that");
    }
    for (const id of params.orderIds) {
        if (id <= 0n || id > 0xffffffffffffffffffffffffffffffffn) {
            throw new Error(`buildCancelOrdersTx: every orderId must be in [1, u128::MAX] (got ${id})`);
        }
    }
    const tx = new Transaction();
    tx.moveCall({
 ...
 */
export function createMarketDeepBookClient(client, address, poolId, poolKey = PREDICT_DEEPBOOK_POOL_KEY, balanceManagerId) {
    const marketConfig = {
        poolKey,
        poolId,
 ...
 */
export function buildVaultDepositTx(vaultId, coinId, amountAtoms, quoteType = DUSDC_TYPE, recipient) {
    ...
    if (amount <= 0n) {
        throw new Error(`buildVaultDepositTx: amountAtoms must be > 0 (got ${amountAtoms})`);
    }
    ...
    if (recipient !== undefined && recipient !== "@{sender}") {
        if (!isValidSuiAddress(recipient)) {
            throw new Error(`buildVaultDepositTx: recipient is not a valid Sui address (got ${recipient.slice(0, 16)}...)`);
        }
    }
    const tx = new Transaction();
 ...
 */
export function buildVaultWithdrawTx(vaultId, vlpCoinId, quoteType = DUSDC_TYPE) {
    const tx = new Transaction();
 ...
 */
export function buildCreateVaultTx(vlpTreasuryCapId, quoteType = DUSDC_TYPE) {
    const tx = new Transaction();
    tx.moveCall({
 ...
 * @param quoteType  - Quote coin type (e.g. DUSDC_TYPE or DBUSDC_TYPE)
 */
export function buildAllocateForMmTx(vaultId, amount, quoteType = DUSDC_TYPE) {
    // R54 audit fix: validate `amount > 0` at the build boundary.
    ...
    if (amount <= 0n) {
        throw new Error(`buildAllocateForMmTx: amount must be > 0 (got ${amount})`);
    }
    const tx = new Transaction();
 ...
 * @param quoteType  - Quote coin type (e.g. DUSDC_TYPE or DBUSDC_TYPE)
 */
export function buildReturnFromMmTx(vaultId, coinId, quoteType = DUSDC_TYPE) {
    const tx = new Transaction();
    tx.moveCall({
 ...
 */
export function buildRegisterMarketTx(registryId, marketObjectId) {
    ...
    if (normalizeObjectId(registryId) === normalizeObjectId(marketObjectId)) {
        throw new Error("buildRegisterMarketTx: registryId and marketObjectId must differ; " +
            "a copy-paste of the same id is always a caller bug");
    }
    const tx = new Transaction();
    tx.moveCall({
 ...
 */
export async function findExistingYesPool(client, deepbookRegistryId, marketPackageId = PREDICT_MARKET_PACKAGE_ID, quoteType = DUSDC_TYPE, fallbackPoolId) {
    ...
    const decodeTypeNameBcs = (bytes) => {
        ...
        if (offset + length > bytes.length) {
            throw new Error(`decodeTypeNameBcs: truncated String (declared length ${length}, ` +
    ...
    const resolveIdAndName = (f) => {
        if (typeof f !== "object" || f === null)
            return undefined;
        const o = f;
        const id = (typeof o["fieldId"] === "string" && o["fieldId"]) ||
    ...
    for (let page = 0; page < MAX_PAGES; page++) {
        ...
        if (typeof g.listDynamicFields === "function") {
            const r = await g.listDynamicFields({
            ...
            const r = await g.getDynamicFields({
                parentId: deepbookRegistryId,
                limit: PAGE_SIZE,
                cursor: cursor,
            });
            ...
            throw new Error("findExistingYesPool: client exposes neither listDynamicFields nor getDynamicFields");
        }
        for (const f of rows) {
        ...
        if (!nextCursor) {
            throw new Error("findExistingYesPool: fullnode returned hasNextPage=true without a cursor; " +
    ...
    // R-WC-3.3 diagnostic: emit a unique marker
    // `###V3PROBE###` + `MARKER-A:` `MARKER-B:` pairs
    // so we can identify the call site even if the
    // rest of the log line is filtered/truncated by
    // the Railway log pipeline.
    process.stdout.write(`###V3PROBE### MARKER-A:${deepbookRegistryId} MARKER-B:${marketPackageId} ` +
        `MARKER-C:${!!knownRegistryPools} MARKER-D:${!!knownPoolId}\n`);
    process.stderr.write(`###V3PROBE-STDERR### MARKER-A:${deepbookRegistryId} MARKER-B:${marketPackageId} ` +
        `MARKER-C:${!!knownRegistryPools} MARKER-D:${!!knownPoolId}\n`);
    console.error(`###V3PROBE-CERR### MARKER-A:${deepbookRegistryId} MARKER-B:${marketPackageId}`);
    if (knownPoolId) {
 ...
 *                                  create_market path)
 */
export async function ensureMarketCreated(client, signer, deepbookRegistry, params) {
    ...
    if (deepbookRegistry) {
        try {
            ...
            if (marketId) {
                ...
                const poolId = await extractCreatedObjectId(client, result.digest, "Pool");
                const balanceManagerId = await extractCreatedObjectId(client, result.digest, "BalanceManager");
            ...
            const isPoolExists = /EPoolAlreadyExists/.test(msg) ||
    ...
    if (!deepbookRegistry) {
        throw new Error("ensureMarketCreated: pool already exists but no deepbookRegistry configured; " +
    ...
    const existingPoolId = await findExistingYesPool(client, deepbookRegistry, undefined, undefined, 
    ...
    if (!existingPoolId) {
        throw new Error("ensureMarketCreated: pool already exists (per abort) but findExistingYesPool " +
    ...
    const marketId = await extractCreatedObjectId(client, result.digest, "PredictionMarket");
    if (!marketId) {
        throw new Error(`ensureMarketCreated: PredictionMarket object not found in effects (digest ${result.digest})`);
    }
    const balanceManagerId = await extractCreatedObjectId(client, result.digest, "BalanceManager");
    return {
        marketId,
...
//# sourceMappingURL=prediction-market-client.js.map