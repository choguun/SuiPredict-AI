...
export async function findExistingYesPool(client, deepbookRegistryId, marketPackageId = PREDICT_MARKET_PACKAGE_ID, quoteType = DUSDC_TYPE, fallbackPoolId) {
    ...
    console.warn(`[findExistingYesPool] v3-lookup: registryInTable=${!!knownRegistryPools} poolInTable=${!!knownPoolId}`);
    if (knownPoolId) {
...
export async function ensureMarketCreated(client, signer, deepbookRegistry, params) {
    ...
    console.warn(`[ensureMarketCreated:diag] create_market_with_pool digest=${result.digest} ` +
    ...
    console.warn(`[ensureMarketCreated:diag] create_market_with_pool returned marketId=${marketId} digest=${result.digest}`);
    const balanceManagerId = await extractCreatedObjectId(client, result.digest, "BalanceManager");
    return {
        marketId,
        poolId: existingPoolId,
