 ...
 * mismatch.
 */
export function buildCreateParlayPoolTx(maxPayoutBps, coinType) {
    ...
    if (bps < BigInt(BPS)) {
        throw new Error(`buildCreateParlayPoolTx: maxPayoutBps must be >= BPS (${BPS}), got ${maxPayoutBps}`);
    }
 ...
 */
export function buildFundParlayPoolTx(poolId, coinId, amountAtoms, coinType) {
    const amount = BigInt(amountAtoms);
    if (amount <= 0n) {
        throw new Error(`buildFundParlayPoolTx: amountAtoms must be > 0 (got ${amountAtoms})`);
    }
    const tx = new Transaction();
 ...
 */
export function buildParlayAdminWithdrawTx(poolId, amount, coinType) {
    const amt = BigInt(amount);
    if (amt <= 0n) {
        throw new Error(`buildParlayAdminWithdrawTx: amount must be > 0 (got ${amount})`);
    }
    const tx = new Transaction();
 ...
 */
export function buildRotateParlayAdminTx(poolId, newAdmin, coinType) {
    ...
    if (!isValidSuiAddress(newAdmin)) {
        throw new Error(`buildRotateParlayAdminTx: newAdmin must be a non-zero Sui address (got "${newAdmin}")`);
    }
    const tx = new Transaction();
    tx.moveCall({
 ...
 * `typeArguments: [coinType]`.
 */
export function buildSetMaxPayoutBpsTx(poolId, newMaxBps, coinType) {
    ...
    if (BigInt(newMaxBps) < BigInt(BPS)) {
        throw new Error(`buildSetMaxPayoutBpsTx: newMaxBps must be >= BPS (${BPS}) (got ${newMaxBps})`);
    }
    const tx = new Transaction();
    tx.moveCall({
...
export function buildCreateParlayTx(args) {
    if (args.marketIds.length !== args.predictions.length) {
        throw new Error(`buildCreateParlayTx: marketIds (${args.marketIds.length}) and ` +
    ...
    if (args.marketIds.length < MIN_LEGS || args.marketIds.length > MAX_LEGS) {
        throw new Error(`buildCreateParlayTx: marketIds must have ${MIN_LEGS}–${MAX_LEGS} legs (got ${args.marketIds.length})`);
    }
    ...
    if (!args.predictions.every((p) => p === 1 || p === 2)) {
        const bad = args.predictions
            ...
            .filter((i) => i !== null);
        throw new Error(`buildCreateParlayTx: predictions must be 1 (YES) or 2 (NO) for every leg ` +
            `(bad indices: ${bad.join(", ")})`);
    }
    const collateral = BigInt(args.collateralAtoms);
    if (collateral <= 0n) {
        throw new Error(`buildCreateParlayTx: collateralAtoms must be > 0 (got ${args.collateralAtoms})`);
    }
    ...
    if (payoutBps <= BigInt(BPS)) {
        throw new Error(`buildCreateParlayTx: payoutBps must be > BPS (${BPS}) (got ${args.payoutBps})`);
    }
    const tx = new Transaction();
...
export function buildRecordLegTx(args) {
    ...
    if (typeof args.legIndex === "number" && !Number.isInteger(args.legIndex)) {
        throw new Error(`buildRecordLegTx: legIndex must be an integer (got ${args.legIndex})`);
    }
    const legIndex = BigInt(args.legIndex);
    if (legIndex < 0n || legIndex > 0xffffffffffffffffn) {
        throw new Error(`buildRecordLegTx: legIndex must be in [0, u64::MAX] (got ${args.legIndex}); ` +
...
// Reads
// ============================================================
async function readParlayObject(client, objectId) {
...
export async function readParlayPoolBalance(client, poolId) {
...
export async function readParlayPayoutBps(client, parlayId) {
    const fields = await readParlayObject(client, parlayId);
    return BigInt(fields?.payout_bps ?? 0);
}
/** Fetch the `owner` (address) for a `Parlay<Q>`. */
export async function readParlayOwner(client, parlayId) {
    const fields = await readParlayObject(client, parlayId);
    return fields?.owner ?? "";
}
/** Fetch the `legs_recorded` (u64) for a `Parlay<Q>`. */
export async function readParlayLegsRecorded(client, parlayId) {
    const fields = await readParlayObject(client, parlayId);
    return BigInt(fields?.legs_recorded ?? 0);
}
/** Fetch the `legs_lost` (u64) for a `Parlay<Q>`. */
export async function readParlayLegsLost(client, parlayId) {
    const fields = await readParlayObject(client, parlayId);
    return BigInt(fields?.legs_lost ?? 0);
}
//# sourceMappingURL=parlay-client.js.map