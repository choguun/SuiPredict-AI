...
export function normalizeCountryCode(code) {
    ...
    if (code.length > MAX_COUNTRY_BYTES)
        return null;
    if (!/^[A-Za-z]+$/.test(code))
        return null;
 ...
 */
export function buildCreateProfileTx(registryId) {
 ...
 */
export function buildSetCountryCodeTx(profileId, countryCode) {
    const tx = new Transaction();
    const normalized = normalizeCountryCode(countryCode);
    if (normalized == null) {
        throw new Error(`countryCode must be 1-${MAX_COUNTRY_BYTES} ASCII letters, got "${countryCode}"`);
    }
 ...
 */
export function buildSetForecasterKindTx(profileId, kind) {
...
client, profileId) {
    ...
    return {
        ...
        created_at_ms: u64ToSafeNumber(fields.created_at_ms ?? 0, "created_at_ms", profileId),
    };
}
/**
 ...
 */
function decodeCountryCodeField(raw) {
    if (raw == null)
        return "";
    if (Array.isArray(raw)) {
        ...
        try {
            return new TextDecoder("utf-8", { fatal: true }).decode(Uint8Array.from(raw));
        }
        catch {
            ...
            try {
                return Buffer.from(Uint8Array.from(raw)).toString("base64");
            }
            catch {
                return "";
            }
