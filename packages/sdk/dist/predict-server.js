import { PREDICT_OBJECT_ID, resolvePredictServerUrl, } from "./constants.js";
...
// error.
function requireId(id, paramName) {
    if (!isValidSuiAddress(id)) {
        throw new Error(`predict-server: ${paramName} must be a non-zero Sui address (got "${id}")`);
    }
...
async function fetchJson(path) {
    ...
    if (len > 5_000_000) {
        throw new Error(`predict-server ${path}: response too large (Content-Length ${len} > 5_000_000)`);
    }
    const ct = res.headers.get("content-type") ?? "";
    if (!ct.includes("application/json")) {
        throw new Error(`predict-server ${path}: expected application/json, got ${ct || "(none)"}`);
    }
    return res.json();
}
export function normalizeOracleState(raw) {
...
export function normalizeVaultSummary(raw) {
    return {
...
export async function getOracles(predictId = PREDICT_OBJECT_ID) {
    return fetchJson(`/predicts/${requireId(predictId, "predictId")}/oracles`);
}
export async function getActiveOracles(predictId = PREDICT_OBJECT_ID) {
...
export async function getOracleState(oracleId) {
    const raw = await fetchJson(`/oracles/${requireId(oracleId, "oracleId")}/state`);
    return normalizeOracleState(raw);
}
export async function getVaultSummary(predictId = PREDICT_OBJECT_ID) {
    const raw = await fetchJson(`/predicts/${requireId(predictId, "predictId")}/vault/summary`);
    return normalizeVaultSummary(raw);
}
export async function getManagers() {
    return fetchJson("/managers");
}
export async function getManagerForOwner(owner) {
    ...
    const mine = managers.filter((m) => typeof m.owner === "string" && m.owner.toLowerCase() === owner.toLowerCase());
    if (mine.length === 0)
        return null;
    return mine[0].manager_id;
}
export async function getManagerSummary(managerId) {
    return fetchJson(`/managers/${requireId(managerId, "managerId")}/summary`);
}
export async function getManagerPositions(managerId) {
    const data = await fetchJson(`/managers/${requireId(managerId, "managerId")}/positions/summary`);
    if (Array.isArray(data))
        return data;
    return data.positions ?? [];
}
export async function getMintedPositions(limit = 50) {
...
export async function getRedeemedPositions(limit = 50) {
    if (!Number.isInteger(limit) || limit < 0) {
        throw new Error(`predict-server: limit must be a non-negative integer (got ${limit})`);
    }
    return fetchJson(`/positions/redeemed?limit=${limit}`);
}
export async function getOracleSviLatest(oracleId) {
    return fetchJson(`/oracles/${requireId(oracleId, "oracleId")}/svi/latest`);
}
export async function getOraclePriceLatest(oracleId) {
    return fetchJson(`/oracles/${requireId(oracleId, "oracleId")}/prices/latest`);
}
export async function findNearestActiveOracle(predictId = PREDICT_OBJECT_ID) {
...
export async function findSettledOraclesWithOpenPositions(managerId) {
    ...
    const settledIds = new Set(oracles
        .filter((o) => o.status === "settled")
        .map((o) => o.oracle_id.toLowerCase()));
    return positions.filter((pos) => pos.quantity > 0 && settledIds.has(pos.oracle_id.toLowerCase()));
}
//# sourceMappingURL=predict-server.js.map