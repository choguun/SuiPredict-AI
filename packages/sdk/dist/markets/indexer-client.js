...
export async function getMarket(id) {
    ...
    if (!id || typeof id !== "string") {
        throw new Error(`getMarket: id must be a non-empty string (got ${id === undefined ? "undefined" : JSON.stringify(id)})`);
    }
    if (!/^0x[0-9a-fA-F]{64}$/.test(id)) {
        throw new Error(`getMarket: id must be a 32-byte hex Sui object id (got ${JSON.stringify(id)})`);
    }
...
export async function getMarketOrderBook(id) {
    ...
    if (!id || typeof id !== "string") {
        throw new Error(`getMarketOrderBook: id must be a non-empty string (got ${id === undefined ? "undefined" : JSON.stringify(id)})`);
    }
    if (!/^0x[0-9a-fA-F]{64}$/.test(id)) {
        throw new Error(`getMarketOrderBook: id must be a 32-byte hex Sui object id (got ${JSON.stringify(id)})`);
    }
