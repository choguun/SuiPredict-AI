...
export const DEFAULT_DISTRIBUTION_BPS = [
    5_000, // rank 1: 50%
    3_000, // rank 2: 30%
    1_500, // rank 3: 15%
    500, // rank 4: 5%
...
];
{
    ...
    if (sum !== 10_000) {
        throw new Error(`DEFAULT_DISTRIBUTION_BPS sums to ${sum} bps, expected 10_000. ` +
 ...
 */
export function buildFundPoolTx(poolId, coinId, amountAtoms, prizeCoinType = DUSDC_TYPE) {
    const amount = BigInt(amountAtoms);
    if (amount <= 0n) {
        throw new Error(`buildFundPoolTx: amountAtoms must be > 0 (got ${amountAtoms})`);
    }
    ...
    const [fundCoin] = tx.splitCoins(tx.object(normalizeObjectId(coinId)), [tx.pure.u64(amount)]);
    tx.moveCall({
...
export function buildCreatePoolTx(params) {
    ...
    if (params.seedAtoms < 0n) {
        throw new Error(`buildCreatePoolTx: seedAtoms must be >= 0 (got ${params.seedAtoms})`);
    }
    if (params.initialWeek < 0n) {
        throw new Error(`buildCreatePoolTx: initialWeek must be >= 0 (got ${params.initialWeek})`);
    }
    const tx = new Transaction();
    // R58.2 audit fix: route the optional `prizeCoinType` through
    // `validateCoinType` so an env-var typo surfaces at build time
    ...
    const [seedCoin] = tx.splitCoins(tx.object(normalizeObjectId(params.initialCoinId)), [tx.pure.u64(params.seedAtoms)]);
    tx.moveCall({
...
export function buildClaimPrizeTx(params) {
    ...
    if (normalizedPoolId !== normalizedPoolIdForSig) {
        throw new Error(`buildClaimPrizeTx: poolId (${params.poolId}) !== poolIdForSig (${params.poolIdForSig}); ` +
    ...
    if (!Number.isInteger(params.rank) || params.rank < 1 || params.rank > MAX_RANK) {
        throw new Error(`buildClaimPrizeTx: rank must be an integer in [1, ${MAX_RANK}] (got ${params.rank})`);
    }
    ...
    if (params.rank - 1 >= effectiveDistribution.length) {
        throw new Error(`buildClaimPrizeTx: rank ${params.rank} exceeds distribution length ${effectiveDistribution.length}; ` +
            "the on-chain distribution is too short to cover this rank. " +
            "Use a shorter rank or update the pool's distribution.");
    }
    if (params.amount <= 0n) {
        throw new Error(`buildClaimPrizeTx: amount must be > 0 (got ${params.amount})`);
    }
    ...
    try {
        ...
        throw new Error(`buildClaimPrizeTx: signatureB64 is not valid base64: ${e instanceof Error ? e.message : String(e)}`);
    }
    if (sigBytes.length !== 64) {
        throw new Error(`buildClaimPrizeTx: signatureB64 must decode to 64 bytes (ed25519), got ${sigBytes.length}`);
    }
    const tx = new Transaction();
...
export function buildSettleWeekTx(poolId, adminCapId, weekIndex, prizeCoinType = DUSDC_TYPE) {
    ...
    if (typeof weekIndex !== "bigint" || weekIndex < 0n) {
        throw new Error(`buildSettleWeekTx: weekIndex must be a bigint >= 0 (got ${weekIndex})`);
    }
    // R58.2 audit fix: route the optional `prizeCoinType` through
    // `validateCoinType` so an env-var typo surfaces at build time
    // instead of as an opaque "Invalid type argument" at signature.
...
export function buildRotateWeekTx(poolId, adminCapId, newWeek, prizeCoinType = DUSDC_TYPE) {
    ...
    if (typeof newWeek !== "bigint" || newWeek < 0n) {
        throw new Error(`buildRotateWeekTx: newWeek must be a bigint >= 0 (got ${newWeek})`);
    }
    // R58.2 audit fix: route the optional `prizeCoinType` through
...
export function buildRotatePubkeyTx(prizeAdminId, newPubkey) {
    ...
    if (!(newPubkey instanceof Uint8Array)) {
        throw new Error(`buildRotatePubkeyTx: newPubkey must be a Uint8Array (got ${typeof newPubkey})`);
    }
    if (newPubkey.length !== 32) {
        throw new Error(`buildRotatePubkeyTx: newPubkey must be exactly 32 bytes (ed25519 pubkey), got ${newPubkey.length}`);
    }
    const tx = new Transaction();
    tx.moveCall({
 ...
 */
export function buildRotateAdminTx(prizeAdminId, newAdmin) {
    ...
    if (!isValidSuiAddress(newAdmin)) {
        throw new Error(`buildRotateAdminTx: newAdmin must be a non-zero Sui address (got "${newAdmin}")`);
    }
    const tx = new Transaction();
    tx.moveCall({
...
export function buildSetDistributionTx(poolId, adminCapId, bps, prizeCoinType = DUSDC_TYPE) {
    ...
    if (bps.length === 0) {
        throw new Error(`buildSetDistributionTx: bps must be non-empty (got length 0)`);
    }
    if (bps.length > MAX_RANK) {
        throw new Error(`buildSetDistributionTx: bps vector must be <= MAX_RANK (${MAX_RANK}) elements (got ${bps.length})`);
    }
    for (const b of bps) {
        if (!Number.isInteger(b) || b < 0 || b > BPS) {
            throw new Error(`buildSetDistributionTx: each bps entry must be an integer in [0, ${BPS}] (got ${b})`);
        }
    }
    const sumBps = bps.reduce((acc, b) => acc + BigInt(b), 0n);
    if (sumBps !== BigInt(BPS)) {
        throw new Error(`buildSetDistributionTx: bps must sum to BPS (${BPS}) (got ${sumBps})`);
    }
    // R58.2 audit fix: route the optional `prizeCoinType` through
    // `validateCoinType` for symmetry with the other prize builders.
 ...
 */
export function expectedAmountForRank(weekPrize, rank, distribution = DEFAULT_DISTRIBUTION_BPS) {
    if (rank < 1)
        return 0n;
    const idx = rank - 1;
 ...
 */
export function buildClaimMessageBytes(payload) {
...
function writeU64LE(buf, offset, n) {
    for (let i = 0; i < 8; i++) {
 ...
 * digest.
 */
export async function signClaimPayload(signer, payload, keccak256) {
...
//# sourceMappingURL=prize-client.js.map