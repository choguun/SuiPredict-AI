...
const USER_PROFILE_CODES = {
    0: "EProfileExists",
    1: "ENotOwner",
    2: "EInvalidCountry",
    3: "EInvalidForecasterKind",
};
const BADGE_NFT_CODES = {
    0: "ENotStreakOwner",
    1: "EInvalidTier",
};
const VAULT_CODES = {
...
export function extractMoveAbortCode(msg) {
...
export function moveAbortSymbol(module, code) {
...
export function moveAbortSymbolAny(code) {
    for (const m of Object.keys(MODULE_CODES)) {
...
export function isMoveAbortCode(err, module, code) {
    const msg = err instanceof Error ? err.message : String(err);
    if (moveAbortSymbol(module, code) === null) {
        throw new Error(`isMoveAbortCode: code ${code} is not defined for module "${module}"`);
    }
...
export function isMoveAbortInModule(err, module) {
    const msg = err instanceof Error ? err.message : String(err);
 ...
 *  to streak ownership). */
export function isMoveAbortSymbol(err, symbol) {
    const msg = err instanceof Error ? err.message : String(err);
    return msg.includes(symbol);
}
//# sourceMappingURL=move-errors.js.map