...
export function highestEligibleBadgeTier(longestStreak) {
...
export function buildMintBadgeTx(args) {
    ...
    if (!Number.isInteger(args.tier) || args.tier < 1 || args.tier > 5) {
        throw new Error(`buildMintBadgeTx: tier must be an integer in [1, 5] (got ${args.tier})`);
    }
...
export function buildMintBadgeToKioskTx(args) {
    ...
    if (!Number.isInteger(args.tier) || args.tier < 1 || args.tier > 5) {
        throw new Error(`buildMintBadgeToKioskTx: tier must be an integer in [1, 5] (got ${args.tier})`);
    }
    const tx = new Transaction();
    tx.moveCall({
...
client, badgeId) {
    ...
    const fields = object?.json ?? undefined;
    if (!fields)
        return null;
    return {
        ...
        threshold_days: u64ToSafeNumber(fields.threshold_days ?? 0, "threshold_days", badgeId),
        longest_streak_at_mint: u64ToSafeNumber(fields.longest_streak_at_mint ??
            0, "longest_streak_at_mint", badgeId),
        minted_at_ms: u64ToSafeNumber(fields.minted_at_ms ?? 0, "minted_at_ms", badgeId),
    };
}
//# sourceMappingURL=badge-nft-client.js.map