...
async function fetchJson<T>(
  ...
  if (!res.ok) {
    ...
    const err = new Error(
      `faucet ${path}: HTTP ${res.status}${body ? " — " + body.slice(0, 256) : ""}`,
...
export async function getFaucetInfo(): Promise<FaucetInfo> {
  return fetchJson<FaucetInfo>("/faucet/info");
}
 ...
 *  "faucet disabled" etc.). */
export async function requestFaucetDusdc(
  params: RequestFaucetDusdcParams,
  ...
  if (!/^0x[0-9a-fA-F]{64}$/.test(params.recipient)) {
    throw new Error(
      `requestFaucetDusdc: recipient must be a 32-byte Sui address (got "${params.recipient}")`,
    );
  }
  ...
  return fetchJson<FaucetMintResponse>("/faucet/dusdc", {
    method: "POST",
