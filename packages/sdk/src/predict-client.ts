...
export async function executeTransaction(
  ...
  const factory: () => Transaction | Promise<Transaction> =
    typeof txOrFactory === "function" ? txOrFactory : () => txOrFactory;
  ...
  const pinFreshGasCoin = async (tx: Transaction): Promise<Transaction> => {
    ...
    if (coins.length === 0) {
      throw new Error(
        `executeTransaction: agent ${owner} has no SUI gas coins; ` +
  ...
  let tx: Transaction = await pinFreshGasCoin(await factory());
  ...
  for (let attempt = 0; attempt <= MAX_RETRY; attempt++) {
    try {
      ...
      const isTransient =
        /(429|TooManyRequests|408|502|503|504|fetch failed|ETIMEDOUT|ECONNRESET|ECONNREFUSED|socket hang up|Service Unavailable|Bad Gateway|Gateway Timeout|Request timeout|Too Many Requests)/i.test(
          msg,
        ) ||
        ...
        if (isVersionRace || /Invalid\s+withdraw\s+reservation|is\s+less\s+than\s+requested/i.test(msg)) {
          try {
            ...
            try {
              ...
              console.warn(
                `[executeTransaction] re-pin gas coin failed: ${(pinErr as Error).message?.slice(0, 120)}`,
              );
              tx = fresh;
            }
          } catch (factoryErr) {
            console.warn(
              `[executeTransaction] rebuild tx failed: ${(factoryErr as Error).message?.slice(0, 120)}`,
            );
            throw e;
          }
        }
