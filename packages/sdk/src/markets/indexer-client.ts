...
export async function getMarketTrades(
  ...
  if (!isValidMarketId(id)) {
    throw new Error(
      `getMarketTrades: id must be a non-empty path-safe string (got ${id === undefined ? "undefined" : JSON.stringify(id)})`,
    );
  }
  const safeLimit =
    Number.isFinite(limit) && limit > 0 ? Math.min(Math.floor(limit), 200) : 50;
  const env = await fetchJson<{ trades: TradeRecord[] }>(
    `/markets/${encodeURIComponent(id)}/trades?limit=${safeLimit}`,
  );
  return env.trades ?? [];
}
