...
const HARDCODED_GROUPS: WcGroup[] = [
  { letter: "A", teams: [
...
let _groupsCache: { at: number; groups: WcGroup[] } | null = null;
let _scheduleCache: { at: number; matches: WcMatch[] } | null = null;
 ...
 */
export async function loadWorldCupConfig(): Promise<WcGroup[]> {
  ...
  try {
    ...
    console.warn(
      `[wc-fetcher] Wikipedia returned ${groups.length} groups; using hardcoded draw`,
    );
  } catch (err) {
    console.warn(
      `[wc-fetcher] Wikipedia scrape failed: ${err instanceof Error ? err.message : err}`,
    );
  }
 ...
 * ~12 days.
 */
export function buildGroupMatches(groups: WcGroup[]): WcMatch[] {
  ...
  const groupHourOffset: Record<string, number> = {
...
export async function fetchMatchSchedule(): Promise<WcMatch[]> {
...
export async function fetchMatchResult(
  ...
  const url = `https://en.wikipedia.org/w/api.php?action=parse&page=${encodeURIComponent(
    `2026 FIFA World Cup Group ${match.group}`,
  ...
  try {
    ...
    const sectionBlockMatch = text.match(
      new RegExp(
        `<section begin=${sectionId} />([\\s\\S]*?)<section end=${sectionId} />`,
        "i",
      ),
    );
    ...
    const sectionRe = new RegExp(
      `\\|\\s*${match.id}\\s*\\|\\|([^|\\n]+)\\|\\|\\s*\\|\\s*${match.homeCode}\\s*\\|\\|\\s*([0-9]+)\\s*-\\s*([0-9]+)\\s*\\|\\|\\s*${match.awayCode}`,
      "i",
    );
    ...
    if (!m) {
      ...
      const altRe = new RegExp(
        `${match.homeCode}\\b[^|]*\\|\\|\\s*([0-9]+)\\s*\\|\\|\\s*-\\s*\\|\\|\\s*([0-9]+)\\s*\\|\\|\\s*${match.awayCode}`,
        "i",
      );
    ...
    console.warn(
      `[wc-fetcher] ${match.id} fetch error: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
 ...
 */
export function matchWinnerTitle(m: WcMatch): string {
  return `Will ${m.homeName} ${m.homeFlag} beat ${m.awayName} ${m.awayFlag}? (Group ${m.group} MD${matchdayFor(m)})`;
}

export function matchWinnerDescription(m: WcMatch): string {
...
export function matchWinnerResolutionSource(m: WcMatch): string {
...
async function scrapeGroupsFromWikipedia(): Promise<WcGroup[]> {
  const out: WcGroup[] = [];
  for (const letter of "ABCDEFGHIJKL") {
    const url = `https://en.wikipedia.org/w/api.php?action=parse&page=${encodeURIComponent(
      `2026 FIFA World Cup Group ${letter}`,
    )}&prop=wikitext&format=json`;
    try {
      const res = await fetch(url, {
        headers: { "User-Agent": UA },
      ...
      if (teams.length === 4) {
        out.push({ letter: letter as WcGroup["letter"], teams });
      }
