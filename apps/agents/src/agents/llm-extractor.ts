...
export interface WcGroupTeams {
  letter: string;
  teams: Array<{ code: string; name: string; pot: number }>;
}
...
export interface ExtractionResult<T = ExtractionData> {
  ...
  source: string; // URL where the data was scraped
  fetched_at: number; // ms since epoch
  duration_ms: number;
  tokens_used?: number; // estimated
}
...
function cacheKey(url: string, schema: ExtractionSchema): string {
  return `${url}::${schema}`;
}
...
function cacheSet(result: ExtractionResult): void {
 ...
 */
export async function extractFromUrl<T = ExtractionData>(
  url: string,
  schema: ExtractionSchema,
  ...
  if (!opts.bypassCache) {
    ...
    if (cached) return cached as ExtractionResult<T>;
  }
  ...
  try {
    const res = await fetch(url, {
      headers: {
        "User-Agent":
          "SuiPredict-WorldCupBot/1.0 (autonomous; +https://sui.io)",
    ...
    if (!res.ok) {
      console.warn(
        `[llm-extractor] ${url} → HTTP ${res.status} ${res.statusText}`,
      );
      return null;
    }
    html = await res.text();
    ...
    console.warn(
      `[llm-extractor] fetch ${url} failed: ${
  ...
  try {
    ...
    try {
      ...
      console.warn(
        `[llm-extractor] JSON parse failed twice for ${url}: ${
  ...
  cacheSet(result as unknown as ExtractionResult);
  return result;
}
 ...
 */
export function clearExtractionCache(): number {
  const n = cache.size;
  cache.clear();
  return n;
}
 ...
 */
export function cacheStats(): { size: number; max: number; hits: number; misses: number } {
 ...
 */
export async function extractGroupTeams(
  groupLetter: string,
): Promise<WcGroupTeams | null> {
  const result = await extractFromUrl<WcGroupTeams>(
    `https://en.wikipedia.org/wiki/2026_FIFA_World_Cup_Group_${groupLetter}`,
    "WcGroupTeams",
  );
