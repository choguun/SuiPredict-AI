...
function openAgentsDb(): Database.Database {
...
const DEFAULT_SOURCES: Array<{
  url: string;
  schema: "WcGroupTeams" | "WcMatchResult" | "WcFixture" | "WcGroupStandings";
  group?: string; // for "WcGroupTeams" / "WcMatchResult" / "WcGroupStandings"
}> = [
  // Main article
  { url: "https://en.wikipedia.org/wiki/2026_FIFA_World_Cup", schema: "WcGroupStandings" },
];

function buildSourceList(): typeof DEFAULT_SOURCES {
  const sources: typeof DEFAULT_SOURCES = [...DEFAULT_SOURCES];
...
function ensureExtractionTable(): void {
...
function writeExtractionEvent(row: ExtractionEventRow): void {
  try {
    const db = openAgentsDb();
    try {
      db.prepare(
        `INSERT OR REPLACE INTO wc_extraction_events
    ...
    console.warn(
      `[web-extractor] failed to persist event: ${
...
export function recentExtractions(limit = 50): ExtractionEventRow[] {
  try {
    const db = openAgentsDb();
    try {
      return db
        .prepare(
          `SELECT * FROM wc_extraction_events ORDER BY fetched_at DESC LIMIT ?`,
        )
        .all(limit) as ExtractionEventRow[];
    } finally {
      db.close();
    }
  } catch {
...
export async function runWebExtractor(ctx: AgentContext): Promise<AgentResult> {
  ...
  const maxPerTick = safeInt(
    process.env.WC_EXTRACTOR_MAX_PER_TICK ?? "",
    6,
    1,
    20,
  );
  ...
  if (!process.env.MINIMAX_API_KEY) {
    return recordResult("WebExtractor", {
      action: "noop",
      reasoning:
        "MINIMAX_API_KEY not set; web-extractor is dormant. Set the key + AGENT_CRON_WC_EXTRACTOR to enable autonomous cross-source verification.",
      confidence: 95,
    });
  }
  ...
  for (const src of sources) {
    try {
      const result: ExtractionResult | null = await extractFromUrl(
        src.url,
        src.schema,
      );
      if (!result) {
...
// Suppress unused import warning — WcGroupTeams and WcMatchResult
// are re-exported types for the REST endpoint.
