...
interface IssuedNonce {
  /** Hex nonce issued to the client (32 random bytes = 64 hex chars). */
  nonce: string;
  /** Unix-ms epoch when the nonce was issued; used to TTL. */
  issuedAtMs: number;
  /**
   * Set to `true` when the nonce is consumed by a successful
   * signature verification. Once consumed, the nonce cannot
   * be replayed.
   */
  consumed: boolean;
}
 ...
 */
export function issueNonce(user: string): {
  nonce: string;
  message: string;
  expiresAtMs: number;
} {
 ...
 */
export function consumeNonce(nonce: string, user: string): {
  ok: true;
} | { ok: false; reason: "not_found" | "user_mismatch" | "expired" | "already_consumed" } {
  ...
  if (bound !== nonce) return { ok: false, reason: "user_mismatch" };
