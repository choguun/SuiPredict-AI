...
import { DUSDC_TREASURY_CAP_ID, DUSDC_TYPE, executeTransaction, keypairFromPrivateKey, } from "@suipredict/sdk";
...
function json(res, status, body, sideEffecting = false, extraHeaders = {}) {
...
function readFaucetInfo() {
    return {
        ...
        faucetAddress: (process.env.AGENT_PRIVATE_KEY &&
 ...
 */
async function faucetDusdc(req, res, body) {
    if (!isFaucetEnabled()) {
        json(res, 403, {
            error: "Faucet is disabled on this deployment",
            detail: "Set ENABLE_FAUCET=true on the agents service to allow DUSDC mints.",
        });
        return;
    }
    if (!isFaucetConfigured()) {
        json(res, 503, {
            ...
            detail: "DUSDC_TREASURY_CAP_ID is unset on the agents service. Run `pnpm --filter @suipredict/agents bootstrap` or publish the dusdc package first.",
        });
        return;
    }
    ...
    if (!SUI_ADDRESS_RE.test(recipient)) {
        json(res, 400, {
            ...
            detail: "recipient must be a 32-byte Sui address (0x + 64 hex chars).",
        });
        return;
    }
 ...
 */
export function handleFaucetRoute(req, res, url) {
