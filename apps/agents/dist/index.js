    ...
    if (present.length > 0) {
        for (const c of present) {
            ...
            if (SECRET_ENV_VARS.has(c.envVar)) {
                ...
                console.log(`  [ok    ] ${c.envVar.padEnd(28)} (${c.agent})  = [secret ${fingerprint}]`);
                continue;
            }
...
function buildSchedule() {
    ...
    for (const entry of entries) {
        if (entry.cron.trim().split(/\s+/).length !== 5) {
            throw new Error(`[agents] invalid cron expression for ${entry.name}: "${entry.cron}" ` +
...
function bootstrapEnv() {
    ...
    if (canonical && legacyA && canonical !== legacyA) {
        console.warn(`[agents] AGENT_POLICY_PACKAGE_ID (${canonical}) differs from PREDICT_PACKAGE_ID (${legacyA}); using AGENT_POLICY_PACKAGE_ID. ` +
    ...
    if (canonical && legacyB && canonical !== legacyB) {
        ...
        // events.
        console.warn(`[agents] AGENT_POLICY_PACKAGE_ID (${canonical}) differs from MARKET_PACKAGE_ID (${legacyB}).\n` +
    ...
    const deepRegistry = process.env.DEEPBOOK_REGISTRY_ID ?? "0x7c256edbda983a2cd6f946655f4bf3f00a41043993781f8674a7046e8c0e11d1";
    ...
    if (!process.env.DUSDC_TYPE) {
        const pkgId = process.env.NEXT_PUBLIC_DUSDC_PACKAGE_ID ??
...
async function runCycle(ctx) {
    ...
    for (const entry of schedule) {
        try {
            ...
            console.log(`  ${entry.name}: ${result.action}: ${result.reasoning.slice(0, 80)}`);
        }
        catch (err) {
...
function startHealthServer() {
    ...
    const server = createServer(async (req, res) => {
        ...
        if (handleMarketsRoute(req, res, url))
            return;
        if (await handleGamificationRoute(req, res, url))
            return;
        ...
        // inside the handler.
        if (handleFaucetRoute(req, res, url))
            return;
        if (url.pathname === "/health") {
            ...
            res.end(JSON.stringify({
                ...
                profile_registry_id: process.env.PROFILE_REGISTRY_ID ?? "",
                admin_address: process.env.ADMIN_ADDRESS ?? "",
                parlay_admin_id: process.env.PARLAY_ADMIN_ID ?? "",
                deepbook_pool_id: process.env.DEEPBOOK_POOL_ID ?? "",
                deepbook_pool_key: process.env.DEEPBOOK_POOL_KEY ?? "",
                ...
                referral_treasury_address: process.env.REFERRAL_TREASURY_ADDRESS ?? "",
                ts_ms: Date.now(),
            }));
            return;
        }
        ...
        if (url.pathname === "/agents/manifest") {
            ...
            res.end(JSON.stringify(schedule.map((s) => ({
                name: s.name,
                cron: s.cron,
                kind: "primary",
            }))));
...
async function main() {
    ...
    try {
        ...
        if (seeded > 0) {
            console.log(`[agents] Seeded ${seeded} World Cup demo markets (skipped ${skipped}).`);
        }
    }
    catch (err) {
        console.warn(`[agents] World Cup demo seed failed: ${err instanceof Error ? err.message : err}`);
    }
    ...
    if (readLegacyPredict())
        console.log(`[agents] Legacy Predict agents enabled`);
    ...
    console.log(`[agents] Scheduler online (POLL_MS=${Number(process.env.AGENT_POLL_INTERVAL_MS ?? 60_000)})`);
