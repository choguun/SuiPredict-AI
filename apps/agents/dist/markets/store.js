...
export function listMarkets(limit, offset) {
...
export function markMarketResolved(marketId, outcome) {
    ...
    getDb()
        .prepare(`UPDATE markets SET status = 'resolved', outcome = ? WHERE (id = ? OR onchain_market_id = ?) AND status != 'resolved'`)
...
// already in place.
export function sumAllTradeVolume() {
    const row = getDb()
        .prepare(`SELECT COALESCE(SUM(quantity * price_bps / 10000), 0) AS total
       FROM trades`)
        .get();
    const v = row?.total ?? 0;
...
export function countUniqueTraders() {
    const row = getDb()
...
export function getPortfolio(address, limit, offset) {
    let rows;
    if (limit !== undefined && offset !== undefined) {
        rows = getDb()
            .prepare(`SELECT p.*, m.title, m.status, m.outcome, m.onchain_market_id
