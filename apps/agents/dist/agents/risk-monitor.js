import { buildPausePolicyTx, dusdcToDollars, executeTransaction, getPolicyState, } from "@suipredict/sdk";
...
export async function runRiskMonitor(ctx) {
    ...
    const pauseUtilization = safeFloat(process.env.RISK_PAUSE_UTILIZATION, DEFAULT_PAUSE_UTILIZATION, 0, 1);
    ...
    // hot-patch.
    const agentPolicyPkg = process.env.NEXT_PUBLIC_AGENT_POLICY_PACKAGE_ID ??
    ...
    if (utilization >= pauseUtilization) {
        ...
        if (!ctx.policyId) {
            return recordResult("RiskMonitor", {
                ...
                reasoning: `Vault utilization ${(utilization * 100).toFixed(2)}% ` +
...
//# sourceMappingURL=risk-monitor.js.map