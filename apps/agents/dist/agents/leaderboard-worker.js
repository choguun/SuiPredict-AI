 ...
 * endpoint.
 */
import { archiveAndClearAtomic, claimedUsersForWeek, getUserProfilesForUsers, listAllDailyScores, weekIndexFor, weekIndexForDay, } from "../gamification/store.js";
import { recordResult } from "../lib.js";
import { pruneOldDecisions } from "../store.js";
const WEEK_MS = 7 * 86_400_000;
function aggregateWeek(weekIndex, rows) {
    const bucket = new Map();
    for (const r of rows) {
        if (weekIndexForDay(r.day_index) !== weekIndex)
            continue;
    ...
    const out = Array.from(bucket.entries()).map(([user, v]) => ({
        user,
...
export async function runLeaderboardWorker() {
    const now = Date.now();
    ...
    const { archived, cleared } = archiveAndClearTransactional(weekly, cutoffDay);
    ...
    return recordResult("LeaderboardWorker", {
        action: "rollup",
        reasoning: `Week ${priorWeek}: archived ${archived} rows, cleared ${cleared} ` +
 ...
 */
function archiveAndClearTransactional(weekly, cutoffDay) {
 ...
 */
function enrichRows(weekIndex, weekly) {
 ...
 */
export function liveRollup(weekIndex, category) {
    const rows = listAllDailyScores();
 ...
 */
export function countryRollup(weekIndex, code, category) {
    const normalized = code.toLowerCase();
    if (!/^[a-z]{2,8}$/.test(normalized))
        return [];
    const all = liveRollup(weekIndex, category);
    const filtered = all.filter((r) => r.country_code && r.country_code === normalized);
