...
import { executeTransaction, isMoveAbortCode, isMoveAbortSymbol, streakIdForUser, } from "@suipredict/sdk";
import { getSharedClient, getSharedJsonRpcClient, recordResult, retryQuery } from "../lib.js";
import { acquireSweepLock, dayIndexFor, getSweepRun, recordDailyScore, recordDailyScoreIfAbsent, releaseSweepLock, } from "../gamification/store.js";
 ...
 */
function isAlreadyRecordedError(err) {
...
function isTransientError(err) {
    ...
    if (isMoveAbortSymbol(err, "EWrongStreakOwner"))
        return false;
    if (isMoveAbortSymbol(err, "ENotAdmin"))
        return false;
    if (isMoveAbortSymbol(err, "EInvalidOutcome"))
        return false;
    if (isMoveAbortSymbol(err, "EStreakBroken"))
        return false;
    // Catch-all: any other Move abort is also permanent.
    if (err instanceof Error && /MoveAbort/.test(err.message))
        return false;
 ...
 */
function pickUserCategory(markets, marketCategory) {
 ...
 * bad read.
 */
async function readCurrentStreaks(client, streakIds) {
    const out = new Map();
    const ids = streakIds.filter((x) => !!x);
    if (ids.length === 0)
        return out;
    try {
        ...
        console.warn("[streak-sweeper] readCurrentStreaks failed; writing streak_after=0:", e instanceof Error ? e.message : e);
    }
    return out;
}
async function queryAllEvents(client, query, stateKey) {
    const out = [];
    ...
    // `markets/store.ts`.
    let cursor = readCursor(stateKey);
    do {
        const page = await retryQuery(`streakSweeper:${stateKey}`, () => client.queryEvents({
            query,
            cursor,
            limit: 1000,
            order: "ascending",
 ...
 */
async function listAllStreakOwners(client, registryId) {
    if (!registryId)
        return new Map();
    const out = new Map();
    const missingValue = [];
...
function decodeStreakIdValue(value) {
 ...
 */
async function readLastParticipationDays(client, streakIdsByOwner) {
    const out = new Map();
    const ids = Array.from(streakIdsByOwner.values());
    if (ids.length === 0)
        return out;
    const ownerById = new Map();
    for (const [owner, id] of streakIdsByOwner)
 ...
 */
export async function resolveDayOutcomes(dayIndex) {
    // R53 audit fix: read
    ...
    const dailyMarketCategory = new Map();
    for (const e of createdRaw) {
        ...
        if (id &&
            ...
            const code = (raw === 1 || raw === 2 || raw === 3) ? raw : 0;
            if (!dailyMarketCategory.has(id))
                dailyMarketCategory.set(id, code);
        }
    }
    if (dailyMarketIds.size === 0)
        return [];
    ...
    for (const e of mintedRaw) {
        ...
        if (!id || !user || !dailyMarketIds.has(id))
            continue;
    ...
    for (const [user, markets] of userMarkets) {
        ...
        const allResolved = mintedAll && [...markets].every((m) => resolvedMap.has(m));
        ...
        if (allResolved) {
            for (const m of markets) {
                ...
                const heldOnWinningSide = winning === 1 ? (pos?.yes ?? 0) : (pos?.no ?? 0);
 ...
 */
function appendRecordParticipation(tx, args, 
...
function buildSweepTx(registryId, adminId, users, dayIndex, agentPolicyPkg) {
...
export async function runStreakSweeper(ctx) {
    ...
    if (!STREAK_REGISTRY_ID || !STREAK_ADMIN_ID) {
        return recordResult("StreakSweeper", {
            action: "skip",
            reasoning: "STREAK_REGISTRY_ID / STREAK_ADMIN_ID not configured — sweeper inert.",
        });
    }
    ...
    if (!acquireSweepLock(dayIndex)) {
        ...
        return recordResult("StreakSweeper", {
            action: "skip",
            reasoning: `Sweep for day ${dayIndex} already running ` +
    ...
    try {
        ...
        try {
            ...
            console.warn("[streak-sweeper] pre-poll position indexer failed, continuing with possibly-stale positions:", err instanceof Error ? err.message : err);
        }
        ...
        for (let i = 0; i < combined.length && !aborted; i += PTB_BATCH) {
            ...
            const tx = buildSweepTx(STREAK_REGISTRY_ID, STREAK_ADMIN_ID, batch, BigInt(dayIndex), process.env.AGENT_POLICY_PACKAGE_ID ?? "");
            try {
                ...
                const streaks = await readCurrentStreaks(client, batch.map((u) => u.streakId));
                ...
                console.log(`[streak-sweeper] batch ${i / PTB_BATCH + 1} → ${res.digest}`);
            }
            catch (err) {
                ...
                // as failed.
                console.warn(`[streak-sweeper] batch ${i / PTB_BATCH + 1} failed, falling back to per-user:`, err instanceof Error ? err.message : err);
                for (const u of batch) {
                    if (aborted)
                        break;
                    ...
                    if (Date.now() - sweepStartMs > PER_USER_BATCH_TIMEOUT_MS) {
                        console.warn(`[streak-sweeper] per-user fallback exceeded ${PER_USER_BATCH_TIMEOUT_MS}ms; aborting sweep.`);
                        aborted = true;
                        break;
                    }
                    const singleTx = buildSweepTx(STREAK_REGISTRY_ID, STREAK_ADMIN_ID, [u], BigInt(dayIndex), process.env.AGENT_POLICY_PACKAGE_ID ?? "");
                    let outcome = { kind: "failed", err: null };
                    ...
                    if (outcome.kind === "ok") {
                        ...
                        const streaks = await readCurrentStreaks(client, [u.streakId]);
                        recordDailyScore({
                            user: u.user,
                            day_index: dayIndex,
                            participated: u.outcome === OUTCOME_NOT_SUBMITTED ? 0 : 1,
                        ...
                        const msg = outcome.err instanceof Error
                            ? outcome.err.message
                            : String(outcome.err);
                        failed++;
                        ...
                        if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
                            console.error(`[streak-sweeper] ${consecutiveFailures} consecutive identical failures — aborting sweep. ` +
                                `Last error: ${msg}`);
                            aborted = true;
                            break;
                        }
        ...
        if (multiDayGaps.length > 0) {
            console.warn(`[streak-sweeper] ${multiDayGaps.length} users had multi-day gaps ` +
                `(NOT_SUBMITTED recorded in daily_scores only; on-chain streak not reset). ` +
                `First few: ${multiDayGaps.slice(0, 3).join(", ")}`);
        }
        return recordResult("StreakSweeper", {
            action: "sweep",
            reasoning: `Day ${dayIndex}: ${sent} ok, ${failed} failed ` +
...
//# sourceMappingURL=streak-sweeper.js.map