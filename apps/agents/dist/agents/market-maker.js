...
export async function runMarketMaker(ctx) {
    ...
    // `[1, 1e15]` atoms.
    const SPREAD_THRESHOLD_BPS = safeInt(process.env.MM_SPREAD_THRESHOLD_BPS, 400, 1, 10_000);
    const QUOTE_SIZE = safeInt(process.env.MM_QUOTE_SIZE, 10_000_000, 1, 1_000_000_000_000_000);
    const BALANCE_MANAGER_ID = loadBalanceManagerId();
    ...
    const PER_ORDER_NOTIONAL_DOLLARS = Math.max(1, Math.round((QUOTE_SIZE * (SPREAD_THRESHOLD_BPS / 10_000 + 0.05)) / 1_000_000));
    ...
    const cycleAuthDollars = Math.min(PER_ORDER_NOTIONAL_DOLLARS * 2, ctx.maxBudgetUsdc);
    // Find an active market that has a DeepBook pool
    const target = listMarkets().find((m) => m.status === "active" && m.deepbook_pool_id && m.category !== "worldcup");
    if (!target) {
    ...
    const TICK = 1000000n; // pool tick_size
    try {
        ...
        if (!dusdcId) {
            ...
            dusdcId = freshCoins.find((c) => BigInt(c.balance) >= 1000000n)?.objectId;
            if (!dusdcId)
                throw new Error("Failed to mint DUSDC");
        }
