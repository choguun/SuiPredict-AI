 ...
 */
import { executeTransaction, DUSDC_TYPE, buildFundPoolTx, buildSettleWeekTx, isMoveAbortInModule, isMoveAbortSymbol, listAllCoins, readPrizePoolWeeklyPrize, } from "@suipredict/sdk";
import { getSharedClient, recordResult, safeBigInt } from "../lib.js";
import { isPoolWeekSettled, markPoolWeekSettled, weekIndexFor, } from "../gamification/store.js";
...
export async function runPrizeAdmin(ctx) {
    ...
    const prizeFundAmount = safeBigInt(process.env.PRIZE_FUND_AMOUNT ?? process.env.PRIZE_WEEKLY_AMOUNT, 0n);
    const prizePoolMinBalance = safeBigInt(process.env.PRIZE_POOL_MIN_BALANCE, 0n);
    ...
    try {
        ...
        const totalDusdc = dusdcCoins.reduce((acc, c) => acc + BigInt(c.balance), 0n);
        if (totalDusdc < prizePoolMinBalance) {
            ...
            if (eligible) {
                ...
                const fundTx = buildFundPoolTx(prizePoolId, eligible.objectId, prizeFundAmount);
                const r = await executeTransaction(client, fundTx, ctx.signer);
                fundedAmount = prizeFundAmount;
                notes.push(`funded ${prizeFundAmount} DUSDC: ${r.digest.slice(0, 12)}…`);
            }
            else {
                notes.push(`no DUSDC coin >= ${prizeFundAmount} — agent balance ${totalDusdc}`);
            }
        }
        else {
        ...
        notes.push(`fund failed: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    try {
        ...
        if (!prizeAdminId) {
            ...
            let priorWeeklyPrize;
            try {
                priorWeeklyPrize = await readPrizePoolWeeklyPrize(client, prizePoolId);
            }
            catch (err) {
                // RPC outage / wrong network / object not found. Don't
                // crash the whole admin pass — log and skip settle.
                notes.push(`settle skipped: could not read weekly_prize ` +
                    `(${err instanceof Error ? err.message : String(err)})`);
                priorWeeklyPrize = -1n;
            }
            if (priorWeeklyPrize === 0n) {
                notes.push(`settle skipped: prior week ${priorWeek} has 0 weekly_prize.`);
            }
                ...
                if (isPoolWeekSettled(prizePoolId, priorWeek)) {
                    notes.push(`settle skipped: prior week ${priorWeek} already settled (mirror).`);
                }
                else {
                    const settleTx = buildSettleWeekTx(prizePoolId, prizeAdminId, BigInt(priorWeek));
                    try {
                        const r = await executeTransaction(client, settleTx, ctx.signer);
                        settledWeek = priorWeek;
                        notes.push(`settled week ${priorWeek} (weekly_prize=${priorWeeklyPrize}): ` +
                            `${r.digest.slice(0, 12)}…`);
                    }
                    catch (settleErr) {
                        ...
                        if (isMoveAbortInModule(settleErr, "prize_pool")) {
                            markPoolWeekSettled(prizePoolId, priorWeek, Date.now());
                            const symbol = isMoveAbortSymbol(settleErr, "EAlreadySettled")
                                ? "EAlreadySettled"
                                : "prize_pool::abort";
                            notes.push(`settle skipped: prior week ${priorWeek} already settled ` +
        ...
        notes.push(`settle failed: ${err instanceof Error ? err.message : String(err)}`);
    }
    const summary = notes.length > 0 ? notes.join("; ") : "no admin actions taken.";
    return recordResult("PrizeAdmin", {
...
//# sourceMappingURL=prize-admin.js.map