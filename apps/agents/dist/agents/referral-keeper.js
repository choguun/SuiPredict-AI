...
import { buildClaimReferralRewardsTx, DUSDC_TYPE, executeTransaction, isMoveAbortSymbol, isValidSuiAddress, listAllCoins, REFERRAL_TREASURY_ADDRESS, } from "@suipredict/sdk";
...
async function listDusdcCoinIds(client, owner) {
    try {
        ...
        console.warn("[referral-keeper] listCoins failed; will skip the forward step:", err instanceof Error ? err.message : err);
        return new Set();
    }
}
/** Build a PTB that transfers the given coin ids to the treasury.
 *  Batched in a single tx so the forward is atomic. */
function buildForwardTx(coinIds, treasury) {
...
export async function runReferralKeeper(ctx) {
    ...
    const markets = listMarkets().filter((m) => m.status === "active" &&
    ...
    for (const r of results) {
        if (!r)
            continue;
        if (r.kind === "claimed")
            claimed.push(r.label);
        else if (r.kind === "noRewards")
            noRewards.push(r.label);
        else
            errors.push(r.label);
    }
    ...
    if (isValidSuiAddress(REFERRAL_TREASURY_ADDRESS)) {
        ...
        if (newCoinIds.length > 0) {
            try {
                ...
                console.warn(`[referral-keeper] forward to treasury ${REFERRAL_TREASURY_ADDRESS} failed:`, err instanceof Error ? err.message : err);
            }
        }
    }
    else {
        console.warn("[referral-keeper] REFERRAL_TREASURY_ADDRESS is unset or the 0x0…000 placeholder — claimed rewards will accumulate in the agent's hot wallet until the env is configured with a real 0x-prefixed 64-char hex address.");
    }
    return recordResult("ReferralKeeper", {
        action: "sweep",
        reasoning: `Claimed ${claimed.length} of ${markets.length} markets${noRewards.length ? ` (${noRewards.length} no-op)` : ""}. ` +
            (forwardedDigest
...
//# sourceMappingURL=referral-keeper.js.map