import { buildResolveMarketTx, executeTransaction, getSpotPrice, findNearestActiveOracle, } from "@suipredict/sdk";
...
async function resolveOutcome(title, resolutionSource, confidenceThreshold) {
    let spot = null;
    try {
        const oracle = await findNearestActiveOracle();
        if (oracle)
            spot = await getSpotPrice(oracle.oracle_id);
    }
    catch {
        /* optional oracle */
    }
...
export async function runMarketResolver(ctx) {
    const now = Date.now();
    ...
    const confidenceThreshold = safeFloat(process.env.RESOLVER_CONFIDENCE, 85, 0, 100);
    const expired = listMarkets().filter((m) => m.status === "active" && m.expiry_ms <= now && m.category !== "worldcup");
    if (expired.length === 0) {
    ...
    const { outcome, confidence, reasoning } = await resolveOutcome(market.title, market.resolution_source, confidenceThreshold);
