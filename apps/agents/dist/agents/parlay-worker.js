 ...
 */
import { buildFinalizeParlayTx, buildRecordLegTx, DUSDC_TYPE, } from "@suipredict/sdk";
...
import { getParlayLegMarketIds, listReadyToFinalizeParlays, listUnfinalizedParlays, } from "../gamification/store.js";
 ...
 */
function isPermanentParlayError(err) {
...
function isTransientError(err) {
    ...
    // module.
    if (isMoveAbortInModule(err, "parlay"))
        return false;
    ...
    if (/module:\s*"[a-z_]+"[\s\S]*MoveAbort/.test(msg))
        return false;
 ...
 */
function buildSingleRecordLegTx(parlayId, marketId, legIndex) {
...
async function recordOneLeg(client, signer, parlayId, marketId, legIndex) {
 ...
 */
export async function runParlayWorker(ctx) {
    ...
    // approaching.
    const sortedByDeadline = [...scoped].sort((a, b) => a.created_at_ms - b.created_at_ms);
    ...
    const MAX_PARLAYS_PER_TICK = safeInt(process.env.MAX_PARLAYS_PER_TICK, 25, 1, 500);
    const scopedAndCapped = sortedByDeadline.length > MAX_PARLAYS_PER_TICK
    ...
    if (scoped.length > sortedByDeadline.length) {
        ...
        console.warn(`[parlay-worker] ${scoped.length - sortedByDeadline.length} parlay(s) ` +
            `deferred to next tick (cap ${MAX_PARLAYS_PER_TICK}).`);
    }
    if (scopedAndCapped.length === 0) {
        return recordResult("ParlayWorker", {
            action: "noop",
            reasoning: allUnfinalized.length === 0
    ...
    for (const parlay of scopedAndCapped) {
        if (parlay.legs_recorded >= parlay.leg_count)
            continue;
        ...
        for (let i = parlay.legs_recorded; i < parlay.leg_count; i++) {
            ...
            const result = await recordOneLeg(client, ctx.signer, parlay.parlay_id, marketId, i);
            if (result.kind === "ok") {
                legsRecorded++;
                console.log(`[parlay-worker] leg ${i} of parlay ${parlay.parlay_id} → ${result.digest}`);
                ...
                legsPermanentFailures++;
                if (sampleFailures.length < 3) {
                    const msg = result.err instanceof Error
                        ? result.err.message
                        : String(result.err);
    ...
    // txs forever.
    const MAX_FINALIZES_PER_TICK = safeInt(process.env.MAX_FINALIZES_PER_TICK, MAX_PARLAYS_PER_TICK, 1, 500);
    ...
    for (const parlay of ready) {
        try {
            ...
            finalized++;
            console.log(`[parlay-worker] finalized parlay ${parlay.parlay_id} → ${res.digest}`);
        }
        catch (e) {
            finalizeFailures++;
    ...
    const reason = `Parlays: ${legsRecorded} legs recorded, ${legsSkippedUnresolved} skipped (unresolved/disputed), ` +
        ...
        (finalizeSample.length > 0
            ...
            : "");
    return recordResult("ParlayWorker", {
        action: legsPermanentFailures > 0 || finalizeFailures > 0 ? "partial" : "advance",
        reasoning: reason,
 ...
 */
async function readParlayLegMarketIds(client, parlayId) {
    try {
        ...
        console.warn("[parlay-worker] readParlayLegMarketIds failed for", parlayId, e instanceof Error ? e.message : e);
        return null;
    }
}
//# sourceMappingURL=parlay-worker.js.map