...
import { DUSDC_TYPE, expectedAmountForRank, DEFAULT_DISTRIBUTION_BPS, signClaimPayload, } from "@suipredict/sdk";
import { weekIndexFor, recordPrizeClaim, getPrizeClaim, getUserProfile, getParlay, getUserWeekRank, listAllParlaysForUser, listUnfinalizedParlaysForUser, listPrizeClaims, listStreakEvents, listDailyScoresForDay, dayIndexFor, } from "./store.js";
 ...
 */
function logAndCorrelate(route, err) {
...
function json(res, status, body, sideEffecting = true, extraHeaders) {
...
export async function handleGamificationRoute(req, res, url) {
    ...
    if (prizePoolMatch) {
        ...
        if (!prizePoolId) {
            json(res, 503, {
                error: "PRIZE_POOL_ID is not configured; run `pnpm bootstrap-gamification` or set NEXT_PUBLIC_PRIZE_POOL_ID in your .env",
            });
            return true;
        }
        try {
            ...
            json(res, 200, {
                prize_pool_id: prizePoolId,
                current_week: safeBigInt(j.current_week != null ? String(j.current_week) : undefined, 0n).toString(),
                weekly_prize: safeBigInt(j.weekly_prize != null ? String(j.weekly_prize) : undefined, 0n).toString(),
                distribution: Array.isArray(j.distribution) ? j.distribution : [],
                balance: safeBigInt(j.balance != null ? String(j.balance) : undefined, 0n).toString(),
            });
            return true;
        }
        catch (err) {
    ...
    if (streakMatch) {
        ...
        const currentStreak = Math.max(last?.final_streak ?? last?.new_streak ?? 0, todayScore?.streak_after ?? 0);
        const longestStreak = Math.max(last?.longest_streak ?? 0, 
    ...
    if (url.pathname === "/stats") {
        try {
            ...
            const { listMarkets, sumAllTradeVolume, countUniqueTraders } = await import("../markets/store.js");
            const markets = listMarkets();
            const active = markets.filter((m) => m.status === "active").length;
            const resolved = markets.filter((m) => m.status === "resolved").length;
            const wc = markets.filter((m) => m.category === "worldcup" && m.status === "active").length;
            let volume = 0n;
            let traders = 0;
            try {
    ...
    if (challengeMatch) {
        ...
        if (!tryRateLimit(`prize-chal:ip:${clientIp(req)}`, {
            capacity: 20,
            refillPerMinute: 20,
        }) ||
            ...
            json(res, 429, { error: "rate limit exceeded; try again later" }, true, { "Retry-After": "12" });
            return true;
        }
    ...
    if (sigMatch) {
        ...
        if (!tryRateLimit(`prize-sig:ip:${clientIp(req)}`, {
            capacity: 10,
            refillPerMinute: 10,
        }) ||
        ...
        // the bound address.
        try {
            await verifyPersonalMessageSignature(new TextEncoder().encode(message), signatureB64, { address: user });
        }
        catch (err) {
        ...
        const { amount, amountSource } = await resolvePrizeAmount(poolId, week, rank);
        if (amount === 0n) {
            json(res, 503, {
                error: "PrizePool weekly_prize is 0 — no funds available for this week. " +
        ...
        try {
            const signed = await signClaimPayload(kp, payload, async (b) => keccak_256(b));
            json(res, 200, {
                payload: {
                    ...signed.payload,
    ...
    if (claimMatch) {
        ...
        if (!tryRateLimit(`prize-claim:ip:${clientIp(req)}`, {
            capacity: 10,
            refillPerMinute: 10,
        })) {
            // R51 audit fix: emit `Retry-After`. The
            // bucket refills at 10/min (6s per token).
            json(res, 429, { error: "rate limit exceeded; try again later" }, true, { "Retry-After": "6" });
            return true;
        }
        //
        ...
        const clientPoolId = typeof body.poolId === "string"
            ...
            ? body.poolId
            : null;
        const weekIndex = typeof body.weekIndex === "string" ? Number(body.weekIndex) : body.weekIndex;
        ...
        if (!claimPoolId) {
            json(res, 400, {
                error: "poolId is required: send a valid poolId in the body or " +
        ...
        const { amount, amountSource } = await resolvePrizeAmount(claimPoolId, weekIndex, body.rank);
        if (amount === 0n) {
            json(res, 503, {
                error: "PrizePool weekly_prize is 0 — no funds available for this week. " +
                    "Run `fund_pool` on-chain to seed it, then retry.",
        ...
        const existing = getPrizeClaim(body.user, weekIndex, claimPoolId);
        if (existing) {
            json(res, 409, {
                error: "prize already claimed for this user and week",
                week_index: weekIndex,
        ...
        try {
            ...
            if (amount > MAX_SAFE) {
                console.warn(`[routes] /prize/claims amount for user=${body.user} ` +
    ...
    const parlayUserMatch = url.pathname.match(/^\/parlay\/user\/(0x[a-fA-F0-9]+)$/);
    if (parlayUserMatch) {
        const addr = parlayUserMatch[1];
        const includeFinalized = url.searchParams.get("include_finalized") === "1";
        ...
        // parlays).
        const allRows = includeFinalized
 ...
 */
async function resolvePrizeAmount(poolId, week, rank) {
    try {
        ...
        console.warn(`[prize/signature] on-chain weekly_prize read failed for ${poolId}:`, err instanceof Error ? err.message : err);
    }
    ...
    return {
        ...
        safeBigInt(process.env.PRIZE_WEEKLY_AMOUNT, 0n), rank, DEFAULT_DISTRIBUTION_BPS),
        amountSource: "env",
    };
}
