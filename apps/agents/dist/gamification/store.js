...
function getDb() {
    ...
    `);
        ...
        try {
            const cols = getDb()
                ...
                .all();
            const hasWidenedPK = cols.length > 0 &&
    ...
    `);
        ...
        try {
            getDb().exec(`ALTER TABLE parlay_legs ADD COLUMN market_id TEXT NOT NULL DEFAULT ''`);
        }
        catch {
            // Column already present; ignore.
        }
        ...
        try {
            getDb().exec(`ALTER TABLE parlays ADD COLUMN payout_source TEXT NOT NULL DEFAULT ''`);
        }
        catch {
            // Column already present; ignore.
        }
        ...
        try {
            const cols = getDb()
                ...
                .all();
            const hasNaturalPK = cols.length > 0 &&
                cols.some((c) => c.pk > 0 &&
...
export function recordStreakEvent(ev) {
    getDb()
        .prepare(`INSERT OR IGNORE INTO streak_events
         (user, kind, new_streak, final_streak, longest_streak,
          multiplier_tier, milestone, day_index, ts_ms)
       VALUES
         (@user, @kind, @new_streak, @final_streak, @longest_streak,
...
export function listStreakEvents(user, limit = 50) {
    const db = getDb();
    if (user) {
        return db
            .prepare(`SELECT * FROM streak_events WHERE user = ? ORDER BY ts_ms DESC LIMIT ?`)
            .all(user, limit);
    }
    return db
        .prepare(`SELECT * FROM streak_events ORDER BY ts_ms DESC LIMIT ?`)
        .all(limit);
}
/** Mark a (pool, week) as settled. Idempotent. */
export function markPoolWeekSettled(poolId, weekIndex, tsMs) {
    ...
    if (!Number.isInteger(weekIndex) || weekIndex < 0 || weekIndex > MAX_WEEK) {
        console.warn(`[gamification.markPoolWeekSettled] rejecting poolId=${poolId} ` +
            `weekIndex=${weekIndex}; must be an integer in [0, ${MAX_WEEK}].`);
        return;
    }
    getDb()
        .prepare(`INSERT INTO pool_weeks (pool_id, week_index, settled, settled_at_ms)
       VALUES (?, ?, 1, ?)
...
export function isPoolWeekSettled(poolId, weekIndex) {
    const row = getDb()
        .prepare(`SELECT settled FROM pool_weeks WHERE pool_id = ? AND week_index = ?`)
        .get(poolId, weekIndex);
    return row?.settled === 1;
}
export function recordDailyScore(score) {
    getDb()
        .prepare(`INSERT INTO daily_scores
 ...
 */
export function recordDailyScoreIfAbsent(score) {
    const res = getDb()
        .prepare(`INSERT OR IGNORE INTO daily_scores
       (user, day_index, participated, all_correct, streak_after, category)
       VALUES (@user, @day_index, @participated, @all_correct, @streak_after, @category)`)
        .run({
        user: score.user,
...
export function listDailyScoresForDay(dayIndex) {
    return getDb()
        .prepare(`SELECT * FROM daily_scores WHERE day_index = ?`)
        .all(dayIndex);
}
export function listAllDailyScores() {
    ...
    return getDb()
        .prepare(`SELECT * FROM daily_scores ORDER BY day_index DESC LIMIT 10000`)
        .all();
}
export function clearDailyScoresBefore(dayIndex) {
    const res = getDb()
        .prepare(`DELETE FROM daily_scores WHERE day_index < ?`)
        .run(dayIndex);
    return Number(res.changes ?? 0);
}
export function archiveWeekly(rows) {
    const insert = getDb().prepare(`INSERT INTO weekly_archive
 ...
 */
export function archiveAndClearAtomic(weekly, cutoffDay) {
    const insert = getDb().prepare(`INSERT INTO weekly_archive
     (user, week_index, score, rank, correct_days, longest_streak, category)
     VALUES (@user, @week_index, @score, @rank, @correct_days, @longest_streak, @category)
     ON CONFLICT(user, week_index) DO UPDATE SET
       score=excluded.score, rank=excluded.rank, correct_days=excluded.correct_days,
       longest_streak=excluded.longest_streak, category=excluded.category`);
    const del = getDb().prepare(`DELETE FROM daily_scores WHERE day_index < ?`);
    const tx = getDb().transaction((items, cutoff) => {
        for (const r of items)
            insert.run(r);
...
export function listWeeklyLeaderboard(weekIndex, limit = 100, category) {
    const baseSelect = `
...
export function getUserWeekRank(user, weekIndex, category = 0) {
    ...
    const row = getDb()
        .prepare(`SELECT w.*, CASE WHEN c.user IS NULL THEN 0 ELSE 1 END AS claimed
       FROM weekly_archive w
       LEFT JOIN prize_claims c
         ON c.user = w.user AND c.week_index = w.week_index
       WHERE w.user = ? AND w.week_index = ? AND w.category = ?`)
        .get(user, weekIndex, category);
    return row ? decorateClaimed(row) : null;
}
function decorateClaimed(row) {
    row.claimed = Boolean(row.claimed);
    return row;
}
export function recordPrizeClaim(claim) {
    ...
    const tx = db.transaction((c) => {
        db.prepare(`INSERT INTO prize_claims
       ...
       ON CONFLICT(pool_id, user, week_index) DO UPDATE SET
         ...
         pool_id=COALESCE(excluded.pool_id, prize_claims.pool_id)`).run(c);
    });
    tx(claim);
}
/**
 ...
 * event payload.
 */
export function getPrizeClaim(user, weekIndex, poolId = "") {
    const row = getDb()
        .prepare(`SELECT * FROM prize_claims
        WHERE pool_id IS ? AND user = ? AND week_index = ?`)
        .get(poolId, user, weekIndex);
    return row ?? null;
}
/**
...
export function listPrizeClaims(weekIndex) {
    if (weekIndex != null) {
        return getDb()
            .prepare(`SELECT * FROM prize_claims WHERE week_index = ? ` +
            `ORDER BY claimed_at_ms DESC LIMIT ?`)
            .all(weekIndex, PRIZE_CLAIMS_LIMIT);
    }
    return getDb()
        .prepare(`SELECT * FROM prize_claims ORDER BY claimed_at_ms DESC LIMIT ?`)
        .all(PRIZE_CLAIMS_LIMIT);
}
/**
 ...
 */
export function claimedUsersForWeek(weekIndex) {
    const rows = getDb()
 ...
 * not milliseconds.
 */
export function weekIndexForDay(dayIndex) {
    return Math.floor(dayIndex / 7);
}
/** All distinct day_indices present in the daily_scores table (ascending). */
export function knownDayIndices() {
    return getDb()
        .prepare(`SELECT DISTINCT day_index FROM daily_scores ORDER BY day_index ASC`)
        .all().map((r) => r.day_index);
}
export function getSweepRun(dayIndex) {
    const row = getDb()
...
dayIndex, staleMs = 6 * 60 * 60 * 1000) {
    ...
    return getDb()
        ...
        getDb()
            .prepare(`INSERT INTO sweep_runs (day_index, status, started_at_ms, finished_at_ms)
           VALUES (?, 'running', ?, NULL)
...
// apparent meaning).
export function releaseSweepLock(dayIndex) {
...
export function upsertUserProfile(profile) {
    ...
    getDb()
        .prepare(`INSERT INTO user_profiles (user, country_code, forecaster_kind, updated_at_ms)
 ...
 */
export function getUserProfilesForUsers(users) {
    const out = new Map();
    if (users.length === 0)
        return out;
    ...
    for (let i = 0; i < users.length; i += CHUNK) {
        ...
        const rows = getDb()
            .prepare(`SELECT * FROM user_profiles WHERE user IN (${placeholders})`)
            .all(...slice);
        for (const r of rows)
            out.set(r.user, r);
    }
...
export function recordParlayLeg(p) {
    ...
    const tx = db.transaction(() => {
        const insert = db
            .prepare(`INSERT OR IGNORE INTO parlay_legs
 ...
 */
export function getParlay(parlayId) {
    const row = getDb()
 ...
 */
export function listUnfinalizedParlays() {
    return getDb()
        .prepare(`SELECT * FROM parlays
        WHERE finalized = 0
 ...
 */
export function listUnfinalizedParlaysForUser(user, limit = 200) {
    return getDb()
        .prepare(`SELECT * FROM parlays
 ...
 */
export function getParlayLegMarketIds(parlayId, legCount) {
    const rows = getDb()
        .prepare(`SELECT leg_index, market_id FROM parlay_legs
        WHERE parlay_id = ?
        ORDER BY leg_index ASC`)
        .all(parlayId);
    if (rows.length === 0)
        return null;
    const out = new Array(legCount).fill("");
    for (const r of rows) {
 ...
 */
export function listAllParlaysForUser(user, limit = 200) {
 ...
 */
export function listReadyToFinalizeParlays() {
    return getDb()
        .prepare(`SELECT * FROM parlays
        WHERE finalized = 0
...
export function recordBadgeMint(b) {
    getDb()
        .prepare(`INSERT OR IGNORE INTO streak_badges
