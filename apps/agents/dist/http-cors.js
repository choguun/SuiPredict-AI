...
function resolveAllowedOrigin() {
    ...
    if (fromEnv) {
        ...
        try {
            ...
            if (u.protocol !== "http:" && u.protocol !== "https:") {
                throw new Error(`[agents] ALLOWED_ORIGIN must be an http(s) URL, got protocol=${u.protocol}`);
            }
            if (u.pathname && u.pathname !== "/") {
                throw new Error(`[agents] ALLOWED_ORIGIN must be an origin (scheme + host[:port]); ` +
                    `got path=${u.pathname}`);
            }
        }
        catch (err) {
            if (err instanceof TypeError) {
                throw new Error(`[agents] ALLOWED_ORIGIN is not a valid URL: ${fromEnv}`);
            }
            throw err;
        }
        return fromEnv;
    ...
    if (process.env.NODE_ENV === "production") {
        throw new Error("[agents] ALLOWED_ORIGIN is required in production. " +
 ...
 */
export function corsFor(sideEffecting) {
    if (!sideEffecting) {
...
//# sourceMappingURL=http-cors.js.map